...
In de prostaatkanker dataset (Sectie -@sec-prostate) zijn veel predictoren aanwezig. 
...
We beperken ons hier tot 10 genen (geselecteerd door het onderzoeksteam op basis van a-priori kennis van de pathways betrokken bij de ontwikkeling van de ziekte). Figuur -@fig-MCXScatter toont de scatter plot matrix. 

```{r fig-MCXScatter,fig.cap='Scatterplot matrix voor de observaties in de bioactiviteit dataset.', out.width='100%', fig.asp=.8, fig.align='center'}
mcx<-read.table("https://raw.githubusercontent.com/statomics/sbc/master/data/mcx.txt",header=TRUE)
head(mcx)
plot(mcx)
```

Figuur -@fig-MCXScatter suggereert dat de BA uitkomst sterk gecorreleerd is met alle genexpressies, maar de figuur toont tevens dat vele genexpressies ook onderling sterk gecorreleerd zijn. Het zal dus waarschijnlijk niet nodig zijn om alle genen simultaan in het model op te nemen. Niettegenstaande de hypothesetest-gebaseerde methoden toepasbaar lijken, argumenteren we hier dat die methoden niet geschikt zijn in de huidige context. 
