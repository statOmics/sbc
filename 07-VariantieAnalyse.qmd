# Variantie analyse {#sec-chap-anova}
...
Een boxplot en QQ-plots voor de data in elke groep worden weergegeven in Figuur -@fig-prostBox en -@fig-prostBox2 respectievelijk.

```{r fig-prostBox,fig.cap='Data-exploratie van het prostacycline niveau bij 36 ratten die behandeld werden met drie verschillende arachidonzuurconcentraties (12 ratten per behandeling). Boxplots van prostacycline niveau in functie van de dosis.', out.width='100%', fig.asp=.8, fig.align='center'}
...
```{r fig-prostBox2,fig.cap='Data-exploratie van het prostacycline niveau bij 36 ratten die behandeld werden met drie verschillende arachidonzuurconcentraties (12 ratten per behandeling). QQ-plot van prostacycline voor lage, matige en hoge dosisgroep.', out.width='100%', fig.asp=.8, fig.align='center'}
prostacyclin |>
...
Figuur -@fig-prostBox geeft weer dat er een effect lijkt te zijn van de arachidonzuurdosis op de hoogte van het prostacycline niveau.
...
Op basis van de boxplots in Figuur -@fig-prostBox zien we dat de variantie gelijk lijkt te zijn tussen de verschillende behandelingsgroepen.
...
De QQ-plots in  Figuur -@fig-prostBox2 tonen geen grote afwijkingen aan van Normaliteit.
...
Zoals bij de t-test kunnen we het probleem ook modelleren a.d.h.v een lineair model door gebruik te maken van dummy variabelen (Sectie -@sec-linDummy).
...
Zoals in Sectie -@sec-linDummy kunnen we het regressie-model opnieuw herschrijven als een model voor elke groep:

1. Voor observaties in **dosisgroep L** wordt het Model -@eq-regmu3

  $$Y_i = \beta_0+\epsilon_i,$$

  met $\epsilon_i \sim N(0,\sigma^2)$.

2. Voor observaties in **dosisgroep M** wordt het Model -@eq-regmu3

  $$Y_i = \beta_0+\beta_1 + \epsilon_i,$$

  met $\epsilon_i \sim N(0,\sigma^2)$.

3. Voor observaties in **dosisgroep H** wordt het Model -@eq-regmu3
...
Gezien Model -@eq-regmu3 een lineair regressiemodel is, kunnen de methoden van lineaire regressie  gebruikt worden voor het schatten van de parameters en hun varianties, het opstellen van hypothesetesten en betrouwbaarheidsintervallen.
...
Net zoals bij enkelvoudige regressie (Sectie -@sec-linAnova) kunnen we opnieuw de kwadratensom van de regressie gebruiken bij het opstellen van de F-test.
...
Net als in Sectie -@sec-linAnova is SSR een maat voor de afwijking tussen de predicties van het anova model (groepsgemiddelden) en het steekproefgemiddelde van de uitkomsten.
Het kan opnieuw geïnterpreteerd worden als een maat voor de afwijking tussen het geschatte Model -@eq-regmu3 en een gereduceerd model met enkel een intercept.
...
Het meet immers de afwijkingen tussen de groepsgemiddelden $\bar Y_j$ en het steekproefgemiddelde $\bar Y$ (Zie Figuur -@fig-prostacSST).
...
```{r fig-prostacSST, fig.cap='Interpretatie van de kwadratensom van de behandeling (SST): de som van de kwadratische afwijkingen tussen de groepsgemiddelden ($\\bar Y_j$) en het steekproefgemiddelde van de uitkomsten ($\\bar Y$)', echo = FALSE}
attach(prostacyclin)
...
met $\text{MST}$ de gemiddelde kwadratensom van de behandeling met $g-1$ vrijheidsgraden en $\text{MSE}$ de gemiddelde residuele kwadratensom uit het niet-gereduceerde model -@eq-regmu3, deze heeft $n-g$ vrijheidsgraden (met het aantal groepen $g=3$).
...
In Figuur -@fig-prostacF wordt de F-verdeling weergegeven samen met de kritische waarde op het 5% significantie niveau en de geobserveerde F-statistiek voor het prostacycline voorbeeld.

```{r fig-prostacF, fig.cap='Een F-verdeling met 2 vrijheidsgraden in de teller en 33 in de noemer. Het aanvaardingsgebied wordt weergegeven in blauw, de kritische waarde en de verwerpingsregio bij het $\\alpha=5\\%$ niveau in rood, en, de geobserveerde f-waarde  en de p-waarde worden in oranje.', out.width='100%', fig.asp=.8, fig.align='center', echo = FALSE}
grid <- seq(0,17,.01)
...
Voorbeelden van meerdere F-verdelingen met een verschillend aantal vrijheidsgraden in teller en noemer worden weergegeven in Figuur -@fig-ftheo.

```{r fig-ftheo, fig.cap='Meerdere F-verdelingen met een verschillend aantal vrijheidsgraden in de teller en de noemer.', out.width='100%', fig.asp=.8, fig.align='center', echo = FALSE}
...
De betrouwbaarheidsintervallen kunnen ook grafisch worden weergegeven wat handig is als er veel vergelijkingen worden uitgevoerd (zie Figuur -@fig-prostacTukey).

```{r fig-prostacTukey, fig.cap='$95\\%$ experimentsgewijze betrouwbaarheidsintervallen voor de paarsgewijze verschillen in gemiddeld prostacycline niveau tussen alle arachidonzuur dosisgroepen. De BIs zijn gecorrigeerd voor multipliciteit via de Tukey methode.', out.width='100%', fig.asp=.8, fig.align='center'}
model1.mcp |>
  confint() |>
  plot()
```
