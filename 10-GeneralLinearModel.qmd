# Algemeen lineair model {#sec-chap-glm}
...
In dit hoofdstuk breiden we daarom enkelvoudige lineaire regressie (Hoofdstuk -@sec-chap-linReg) uit door meerdere predictoren toe te laten. We zullen dus de gemiddelde uitkomsten modelleren als een functie van meerdere predictoren.
...
```{r fig-lpsaAll,fig.cap='Scatterplot matrix voor de observaties in de prostaat kanker dataset.', out.width='100%', fig.asp=.8, fig.align='center'}
library(GGally)
prostate |>
  dplyr::select(-pgg45)  |>
  ggpairs()
...
Figuur -@fig-lpsaAll toont de scatter matrix van de data en suggereert dat de lpsa sterk positief gecorreleerd is met het volume en svi.
...
```{r fig-prosAdditiveFit,fig.cap='Fit van het additieve model met termen lcavol, lweight en svi. De figuur geeft duidelijk weer dat de gemiddelde lpsa toeneemt i.f.v. het log-tumorvolume, het log-prostaatgewicht en de invasie van de zaadblaasjes. Merk op dat de fit resulteert in twee parallele vlakken, een regressievlak voor patiënten zonder (blauw) en met invasie van de zaadblaasjes (oranje).', out.width='100%', fig.asp=.8, fig.align='center', message=FALSE,echo=FALSE}
library(plot3D)
grid.lines = 10
x<-prostate$lcavol
y<-prostate$lweight
...
Net zoals in Hoofdstuk -@sec-chap-linReg is het op basis van de puntschatters voor de hellingen niet duidelijk of de verbanden werkelijk voorkomen in de populatie of indien we de verbanden door toeval hebben geobserveerd in de dataset.
...
Analoog als in Hoofdstuk -@sec-chap-linReg kunnen we opnieuw toetsen en betrouwbaarheidsintervallen construeren op basis van de teststatistieken  
...
Afwijkingen van lineariteit kunnen opnieuw worden opgespoord d.m.v. een *residuplot*. Deze wordt weergegeven in Figuur -@fig-prosLinDiag1 links boven. Als de veronderstelling van lineariteit opgaat, krijgt men in een residuplot geen patroon te zien.
...
```{r fig-prosLinDiag1,fig.cap='Diagnostische plots voor het nagaan van de veronderstellingen van het lineair regressiemodel waarbij lpsa gemodelleerd wordt a.d.h.v. de predictoren lcavol, lweight en svi.', out.width='100%', fig.asp=.8, fig.align='center', message=FALSE}
par(mfrow=c(2,2))
plot(lmVWS)
```
...
De residu-plot voor het prostaatkanker voorbeeld Figuur -@fig-prosLinDiag1 links boven geeft geen afwijkingen weer van homoscedasiticiteit.
Alle residuen zijn mooi gespreid binnen dezelfde grenzen voor elke gefitte waarde $\hat y_i$.
De plot van de vierkantswortel van de absolute waarde van de gestandardiseerde error $\sqrt{|e_i|/\sqrt{MSE}}$ in functie van de predicties (Figuur -@fig-prosLinDiag1 links onder) geeft ook geen afwijkingen van homoscedasticiteit weer.


### Normaliteit

Opnieuw kunnen we de veronderstelling van normaliteit nagaan door gebruik te maken van QQ-plots.  Figuur -@fig-prosLinDiag1 rechts boven geeft de QQ-plot weer van de residuen voor het prostaatkanker voorbeeld. We zien in de plot geen aanwijzing voor afwijkingen van normaliteit.
...
Het is interessant om in de R output voor het model met lcavol:lweight interactie ook naar schattingen van de hoofdeffecten te kijken (Sectie -@sec-intCont).
...
```{r fig-vetScatter,fig.cap='Scatterplot matrix van de dataset bodyfat.', out.width='100%', fig.asp=.8, fig.align='center', message=FALSE,echo=FALSE}
...
De scatterplot matrix in Figuur -@fig-vetScatter geeft aan dat er multicollineariteit aanwezig is wat betreft de predictoren $X_1$ en $X_2$, maar niet meteen wat betreft de middenarmomtrek $X_3$. Niettemin bekomen we erg hoge VIFs voor alle model parameters.
...
```{r fig-influential,fig.cap='Impact van outliers op de regressie. Regressie zonder outliers (zwart), regressie met outlier 1 (groen), regressie met outlier 2 (rood), regressie met outlier 3 (blauw).', out.width='100%', fig.asp=.8, fig.align='center', message=FALSE}
set.seed(112358)
nobs<-20
sdy<-1
...
```{r fig-prosCooksAdditiveInt,fig.cap='Evaluatie van de invloed van individuele observaties op het additieve model (links) en het model met de lcavol:lweight interactie (rechts).', out.width='100%', fig.asp=.8, fig.align='center', message=FALSE}
par(mfrow=c(2,2))
plot(lmVWS,which=5)
...
```{r fig-prosDfBetasAdditive,fig.cap='DFBETAS voor additieve model in prostaatkanker.', out.width='100%', fig.asp=.8, fig.align='center', message=FALSE}
par(mfrow=c(2,2))
dfbetasPlots(lmVWS)
```


```{r fig-prosDfBetasInt,fig.cap='DFBETAS voor model met lcavol:lweight interactie.', out.width='100%', fig.asp=.8, fig.align='center', message=FALSE}
par(mfrow=c(2,2))
...
```{r fig-prosInfl,fig.cap='Boxplot met outlier voor prostaatgewicht.', out.width='100%', fig.asp=.8, fig.align='center', message=FALSE}
