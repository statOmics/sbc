# Categorische data analyse {#sec-chap-categorisch}
...
Zoals we in Sectie -@sec-kruistabel hebben gezien zijn *kruistabellen* aangewezen om hun associatie voor te stellen.
...
Een grafische weergave van enkele Bernoulli kansverdelingen wordt weergegeven in Figuur -@fig-bernoulli.

```{r fig-bernoulli,fig.cap='Bernoulli verdelingen.', out.width='100%', fig.asp=.8, fig.align='center',echo=FALSE}
par(mfrow=c(1,3))
probs=c(0.25,.5,.75)
...
Een toevalsveranderlijke $S$ met een kansverdeling zoals in Model -@eq-binomk noemt men een *Binomiaal verdeelde toevalsveranderlijke*. De bijhorende kansverdeling is de *Binomiale kansverdeling* met
...
gegeven in Figuur -@fig-binoms.

```{r fig-binoms,fig.cap='Binomiale verdelingen.', out.width='100%', fig.asp=.8, fig.align='center',echo=FALSE}
par(mfrow=c(2,2))
probs=c(0.25,.5,.75)
for (i in 1:length(probs))
{
...
Dit blijkt ook uit Figuur -@fig-pitests.  

```{r fig-pitests,fig.cap='Binomiale verdeling van het aantal jongens S onder $H_0: \\pi=0.5 (n=6155)$.', out.width='100%', fig.asp=.8, fig.align='center',echo=FALSE}
...
De experimentele set-up betreft een rechthoekige doos van plexiglas met drie compartimenten, waarin het vrouwtje zich in het middenste gedeelte bevindt (zie Figuur -@fig-hamsterKooi).

```{r fig-hamsterKooi,fig.cap='Experimentele opstelling voor het bepalen van de partnerkeuze bij dwerghamsters', out.width='100%', fig.asp=1, fig.align='center',echo=FALSE}
library(plotrix)
...
De resultaten van de studie zijn samengevat in de onderstaande kruistabel (Tabel -@tbl-catHamster).
...
basis van het principe in Sectie -@sec-binom.
...
### Ongepaarde gegevens {#sec-catOnPaired}
...
We beschouwen hier opnieuw Voorbeeld -@exm-brcaLeu.
Het is belangrijk om dit voorbeeld eerst grondig door te nemen alsook de Sectie rond data exploratie van categorische variabelen (Sectie -@sec-explCatVar).
...
Informatie omtrent het BRCA1-polymorfisme werd bekomen via DNA-analyse en staat opnieuw getabuleerd in Tabel -@tbl-leu3.

```{r tbl-leu3, tidy=FALSE,echo=FALSE}
...
In Sectie -@sec-retrospect zagen we reeds dat *het relatief risico* op de aandoening (d.w.z. op *case*)
...
In voorbeeld -@exm-brcaLeu konden we daarom de odds op borstkanker berekenen voor vrouwen met allel Leu/Leu
...
Hiertoe zullen we de kruistabel eerst herschrijven tot een 2x2 tabel zodat we de vrouwen met een Leu/Leu variant vergelijken met de vrouwen in de studie die niet homozygoot zijn in de mutatie^[heterozygoot Pro/Leu of homozygoot Pro/Pro] (Tabel -@tbl-leu4).
...
```{r tbl-leu4, tidy=FALSE,echo=FALSE}
...
Figuur -@fig-chidist toont haar algemene vorm.

```{r fig-chidist,fig.cap='Dichtheidsfuncties voor enkele Chi-kwadraat verdelingen', out.width='100%', fig.asp=.8, fig.align='center', echo = FALSE}
grid <- seq(0,10,.1)
plot(grid,
  dchisq(grid, 1),
  type = "l",
...
Net zoals we bij het regressiemodel voor Normale continue gegevens hebben gezien in Hoofdstuk -@sec-chap-linReg, -@sec-chap-anova en -@sec-chap-glm, laten logistische regressie modellen toe om binaire gegevens te modelleren a.d.h.v. continue en/of dummy variabelen.
...
We illustreren logistische regressie aan de hand van het borstkanker voorbeeld waar we bestuderen of de BRCA 1 variant geassocieerd is met het krijgen van borstkanker (zie Sectie -@sec-catOnPaired).
...
De odds ratios die worden bekomen met het logistisch regressiemodel zijn exact gelijk aan de odds ratios die we zouden bekomen op basis van Tabel  -@tbl-leu3:
...
Figuur -@fig-keversFit toont de geschatte probabiliteit in functie van de dosis die werd gefit a.d.h.v. het regressiemodel. De figuur werd bekomen met onderstaande R code.
...
```{r fig-keversFit,fig.cap='Fit van het logistische regressiemodel voor de kevers data. Het aantal kevers die dood/levend was per dosis is weergegeven met een cijfer.', out.width='100%', fig.asp=.8, fig.align='center'}
