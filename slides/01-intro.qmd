---
title: "1. Inleiding: Waarom Statistiek?"
...
![](../figures/ways_to_capture_b2b_customer_data_5bacd2dbe1a09.png)
...
- Experiment:
    ...
    - de personen worden at random verdeeld over twee behandelingsgroepen:
        ...
        - Randomisatie (at random toewijzen van proefpersonen aan de behandeling) is belangrijk om ervoor te zorgen dat de groepen vergelijkbaar zijn.
...
## Random steekproeftrekking (Random sampling)

- Wat zijn de consequenties van het gebruik van een random steekproeftrekking?

- Random steekproeftrekking is sterk gerelateerd met het concept van de populatie en scope van de studie.  
...
- Om te begrijpen dat een steekproef random is zouden we hetzelfde experiment veel keer moeten kunnen herhalen (`herhaalde steekproeftrekking`/`repeated sampling`).
...
We maken nu een subset van de data die we zullen gebruiken om aan te tonen hoe de data in kleine steekproeven kan variëren van steekproef tot steekproef.
...
- lijken een normale verdeling te volgen
- met dezelfde variantie


$\rightarrow$ ze verschillen dus enkel in gemiddelde. 

- Onderzoeksvraag: "Is er een verschil in lengte tussen mannen en vrouwen?" 
- kunnen vertalen naar 1 karakteristiek: het gemiddeld verschil in lengte

```{r}
HeightSum$mean[1]-HeightSum$mean[2]
```

Gemiddeld zijn volwassen Amerikaanse vrouwen 13.8 cm kleiner dan mannen. 
...
### Herhaal het experiment heel veel keer 
...
### Effect van vergroten van steekproef 
...
$\rightarrow$ Bijgevolg zijn onze conclusies ook onzeker en kunnen deze wijzigen van steekproef tot steekproef.

$\rightarrow$ Met statistiek controleren we de kans op het trekken van foute conclusies.

- We zagen ook dat we het werkelijke verschil meer precies kunnen schatten als we de steekproefgrootte vergroten. 

$\rightarrow$ Design van experiment zal dus bepalen hoe makkelijk we het werkelijk verschil op kunnen pikken met de steekproef.
...
- We hebben gezien dat
    ...
    - random steekproeftrekking nodig is om een representatieve steekproef te nemen
