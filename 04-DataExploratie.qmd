# Data exploratie en beschrijvende statistiek  {#sec-chap-describe}
...
^[Zie Sectie -@sec-qq.]) die aan de grond liggen van de voorgestelde
...
```{r fig-pop2Samp2PopDataExpl,echo=TRUE,fig.cap='Verschillende stappen in een studie. In dit hoofdstuk ligt de focus op de data-exploratie en beschrijvende statistiek', out.width='100%', fig.asp=.8, fig.align='center',echo=FALSE}
if ("pi"%in%ls()) rm("pi")
...
```{r fig-fretabs,fig.cap='Staafdiagram van het aantal mannen en vrouwen in de NHANES studie.', out.width='100%', fig.asp=.8, fig.align='center'}
...
beschrijven. In Voorbeeld -@exm-nhanesEx is de variable *Gender*
...
```{r fig-histo,fig.cap='Histogram van het BMI in de NHANES studie.', out.width='100%', fig.asp=.8, fig.align='center'}
NHANES |>
  ggplot(aes(x=BMI)) +
...
Op het histogram in Figuur -@fig-histo worden densiteiten
...
vergelijkingen tussen verschillende groepen toe (zie verder). Een Boxplot voor het BMI wordt getoond in Figuur -@fig-boxBMI. De boxplot toont een
...
```{r fig-boxBMI,fig.cap='Boxplot van BMI in de NHANES studie.', out.width='100%', fig.asp=.8, fig.align='center',echo=FALSE}
...
statistische parameters (zie Sectie -@sec-summarize). Als de conclusies
...
beperkte mate op in (zie Sectie -@sec-summarize, mediaan).
...
gegevens, zoals de Normale verdeling (zie Sectie -@sec-normal). Een
...
```{r fig-histLogBMI,fig.cap='Boxplot van BMI en log(BMI) in de NHANES studie.', out.width='100%', fig.asp=.8, fig.align='center'}
logSummary <-
NHANES |>
...
&nbsp;
:::
Het gemiddelde en mediane BMI bedraagt `r round(mean(BMI),2)` en `r median(BMI)` , respectievelijk. Het gemiddelde is hier groter dan de mediaan omdat de BMI scheef verdeeld is naar rechts (zie Figuur -@fig-histLogBMI). De verdeling wordt meer symmetrisch na log-transformatie. Het gemiddelde en mediane log-BMI liggen ook dichter bij elkaar en bedragen respectievelijk `r round(mean(log(BMI)),2)` en `r round(median(log(BMI)),2)`. De geometrische gemiddelde BMI-concentratie bekomen we door de exponentiële functie te evalueren in `r round(mean(log(BMI)),2)`, hetgeen ons `r round(exp(mean(log(BMI))),2)` oplevert. Merk op dat dit inderdaad beter met de mediaan overeenstemt dan het rekenkundig gemiddelde.
...
dieper in op dit onderscheid in Hoofdstuk -@sec-chap-linReg rond lineaire regressie.
...
verdeelde variabelen - zie Sectie -@sec-normal) bij benadering 68% van
...
::: {#def-bereik-en-interkwartielafstand}
...
kalkgrond^[Merk op dat dit design niet optimaal is omdat replicaties op de verkeerde schaal werden bekomen. Idealiter moesten meer dan 2 stukken grond in de studie opgenomen worden omdat de 2 gekozen stukken grond in veel meer kunnen verschillen dan alleen het bodemtype. Verschillen in de verdeling van houtluizen kunnen bijgevolg niet zomaar aan het bodemtype kunnen toegeschreven worden.]. Tabel -@tbl-cox toont de bekomen gegevens. Hier stelt $a$ ($c$) het aantal houtluizen van de soort Armadilidium (Oniscus) voor op kleigrond, en $b$ ($d$)  het aantal houtluizen van de soort Armadilidium (Oniscus) op kalkgrond.

```{r tbl-cox,tidy=FALSE,echo=FALSE}
...
::: {#def-absolute-risico-verschil}
## absolute risico verschil
...
### Retrospectieve studies {#sec-retrospect}

Beschouw de case-controle studie uit Voorbeeld -@exm-brcaLeu, waarvan
de gegevens samengevat zijn in Tabel -@tbl-leu2. Omdat men in
...
```{r tbl-leu2, tidy=FALSE,echo=FALSE}
...
per 10 verliesbeurten, 9 keer verwacht te winnen. In de genetische associatiestudie uit Voorbeeld -@exm-brcaLeu
...
Op basis van de gegevens in Tabel -@tbl-leu2 kan de odds ratio op
...
Voorbeeld -@exm-brcaLeu de odds op borstkanker voor vrouwen met allel Leu/leu
...
### Associatie tussen twee kwalitatieve variabelen {#sec-kruistabel}
...
```{r tbl-genderBMI, tidy=FALSE,echo=FALSE}
knitr::kable(table(NHANES$Gender,NHANES$BMI_WHO), tableType, caption = 'Kruistabel van Gender vs BMI klasse.',
  booktabs = TRUE
)
```

Tabel -@tbl-genderBMI toont zo'n kruistabel voor het aantal mannen en vrouwen per BMI klasse. Dergelijke eenvoudige kruistabel met slechts 2 rijen en 4 kolommen, noemt men ook een $2\times 4$ tabel.

### Associatie tussen één kwalitatieve en één continue variabele {#sec-asskwalcont}
...
worden getoond in Figuur -@fig-boxplotCholGender.

```{r fig-boxplotCholGender,fig.cap='Boxplot van log-getransformeerde directe HDL cholestorol concentratie in functie van Gender voor alle subjecten van de NHANES studie.', out.width='100%', fig.asp=.8, fig.align='center'}
NHANES |>
...
log-cholestorol concentratie tussen vrouwen en mannen. We zullen in Hoofdstuk -@sec-chap-besluit dieper op deze vraag ingaan.

Figuur -@fig-boxplotCholGender kan men samenvatten door gemiddelde verschillen tussen beide groepen te rapporteren. Hier stellen we een gemiddeld verschil van `r abs(round(lm(log(DirectChol)~Gender,NHANES)$coef[2],2))` in directe HDL cholestorol concentratie vast op de log schaal tussen vrouwen en mannen.
...
Een
voorbeeld hiervan is weergegeven in Figuur -@fig-captoDot. De figuur vat de gegevens samen van de captopril studie die de centrale dataset vormt van Hoofdstuk -@sec-chap-besluit. In de studie wenst men het effect van een bloeddrukverlagend geneesmiddel captopril evalueren. Voor elke patiënt in de studie werd de systolische bloeddruk twee keer gemeten: één keer voor en één keer na de behandeling met het bloeddruk verlagende medicijn captopril. In Figuur -@fig-captoDot worden de metingen van dezelfde patiënt met een lijntje verbonden. Hierdoor krijgen we een heel duidelijk beeld van de gegevens. Namelijk, we krijgen een sterke indruk dat de bloeddruk daalt na het toedienen van captopril gezien we bijna voor alle patiënten een daling observeren.

```{r fig-captoDot,fig.cap='Dotplot van de systolische bloeddruk in de captopril studie voor en na het toedienen van het bloeddruk verlagend middel captopril.', out.width='100%', fig.asp=.8, fig.align='center',echo=TRUE}
...
In de figuur -@fig-bijdrageCor wordt de bijdrage weergegeven van individuele metingen in de correlatie. Als punten in het 1ste en 3de kwadrant liggen is er een positieve bijdrage van de observatie in de correlatie, als ze in het 2de en 4de kwadrant liggen is er een negatieve bijdrage.

```{r fig-bijdrageCor, fig.cap='Bijdrage van individuele metingen in de correlatie.', echo=FALSE}
...
In figuur -@fig-corOutlier wordt de impact van outliers op de Pearson correlatie geïllustreerd d.m.v. gesimuleerde data met één outlier. We zien dat de correlatie bijna halveert ten gevolge van de outlier!

```{r fig-corOutlier, fig.cap='Correlatie van gesimuleerde data met 1 outlier'}
set.seed(100)
x <- rnorm(20)
...
In figuur -@fig-corQuad simuleren we data met een kwadratisch verband en observeren we dat de correlatie bijna nul is!

```{r fig-corQuad, fig.cap="Correlatie van gesimuleerde data met een kwadratisch verband. De data in de bovenste kwadranten compenseren elkaar als het ware alsook in de onderste kwadranten (ongeveer evenveel positieve en negatieve bijdragen door de data in de correlatie)"}
x <- rnorm(100)
