...
```{r pop2Samp2PopDesign,echo=TRUE,fig.cap='Verschillende stappen in een studie. In dit hoofdstuk ligt de focus op proefopzet.', out.width='100%', fig.asp=.8, fig.align='center',echo=FALSE}
...
```{definition,name="incidentie en prevalentie"}
&nbsp;
```
...
```{example, name="De nood aan controle"}
&nbsp;
```
...
```{definition,name="confounding en confounder"}
&nbsp;
```
...
```{example, name="Confounding in mariene veldexperimenten"}
&nbsp;
```
...
```{definition,name="gerandomiseerde studie"}
&nbsp;
```
...
```{r nfipStudy, tidy=FALSE,echo=FALSE}
...
```{r dbrcStudy, tidy=FALSE,echo=FALSE}
...
```{definition,name="systematische allocatie"}
&nbsp;
```
...
```{example, name="Confounding in mariene veldexperimenten, vervolg"}
&nbsp;
```
...
```{definition,name="gestratificeerde randomisatie"}
&nbsp;
```
...
```{example, name="Oxidatieve stress in Arabidopsis"}
&nbsp;
```
...
```{example, name="Effect van bladschade"}
&nbsp;
```
...
```{example, name="Kiezelwieren en zware metalen"}
&nbsp;
```
...
```{example, name="Competitie tussen species lage begroeiing"}
&nbsp;
```
...
```{example, name="Effect van koper op neerstrijken van larven"}
&nbsp;
```
...
```{definition,name="interactie of effect-modificatie"}
&nbsp;
```
...
```{example, name="Grootte van salamanderlarven"}
&nbsp;
```
...
```{example, name="Groei van esdoorn versus beuk"}
&nbsp;
```
...
```{example, name="Gezondheidscampagne in Wales"}
&nbsp;
```
...
```{example, name="Simpson's paradox"}
&nbsp;
```
...
```{r sexbias, tidy=FALSE,echo=FALSE}
...
```{example, name="Confounders in de NHANES studie"}
&nbsp;
```
...
```{example, name="Observationele versus gerandomiseerde studies"}
&nbsp;
```
...
## Prospectieve studies {#subsec:design:prosp}
...
```{example, name="Vruchtbaarheid van schelpdieren"}
&nbsp;
```
...
## Retrospectieve studies {#subsec:design:retro}
...
```{example, name="Genetische associatiestudies", label="brcaLeu"}
&nbsp;
```
...
### Pre-test/Post-test studies {#subsec:prepost}
