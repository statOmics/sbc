...
Eerst en vooral wil ik het Dodona team bedanken om me herhaaldelijk aan te sporen om ook statistiekonderwijs via Dodona te verstrekken. Prof.~Peter Dawyndt en Dr.~Bart Mesure hebben samen met hun team een indrukwekkend platform ontwikkeld voor het doceren van programmeertalen.
...
\begin{itemize}
\item
  Een html versie van de cursus is beschikbaar op \url{https://statomics.github.io/sbc/} waardoor alle voorbeelden en code in deze cursus makkelijk in R kunnen worden gereproduceerd, wat handig kan zijn wanneer je zelf r-markdown scripts ontwikkelt.
\item
  Een pdf versie van de cursus is beschikbaar op \url{https://statomics.github.io/sbc/Statistiek_2026_2027.pdf}
\item
  Voor een introductie tot R en Data Visualisatie raden we de eerste twee delen aan van het ebook \url{https://rafalab.github.io/dsbook/} van prof. Rafael Irizarry. Deze zijn herwerkt in de interactieve Dodona cursus \url{https://dodona.be/en/courses/6616/}
\item
  De oefeningen vanaf hoofdstuk 5 vind je in de interactieve Dodona cursus \url{https://dodona.be/en/courses/6617/}
\end{itemize}
...
link naar webpage/script die wordt gebruik in de kennisclips: \href{https://statomics.github.io/sbc/rmd/01-intro.html}{script Hoofdstuk1}
...
Ze vervult daarom een belangrijke rol in zowat alle wetenschappen. Zie onder meer de populaire column `'points of significance'\,' in Nature Methods. (\url{http://blogs.nature.com/methagora/2013/08/giving_statistics_the_attention_it_deserves.html})
...
\item
  ...
  \tightlist
  \item
    \emph{Proefopzet (``Experimental Design''):} het ontwerpen van het experiment,
  \item
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/wetMet-1} 
...
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{Statistiek_2026_2027_files/figures/dgge.png}
...
\def\labelenumi{\arabic{enumi}.}
\item
  We pipen (via \texttt{\textbar{}\textgreater{}}) het \texttt{ap} dataframe naar de \texttt{group\_by} functie om de data te groeperen per treatment trt: \texttt{group\_by(trt)}.
\item
...
\NormalTok{apRelSum }\OtherTok{\textless{}{-}}\NormalTok{ ap }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{summarize\_at}\NormalTok{(}\StringTok{"rel"}\NormalTok{, }\FunctionTok{list}\NormalTok{(}\AttributeTok{mean =}\NormalTok{ mean, }\AttributeTok{sd =}\NormalTok{ sd))}
\end{Highlighting}
\end{Shaded}
...
Een uitgebreide introductie tot ggplot vind je in de Dodona module \href{https://dodona.ugent.be/nl/courses/345/}{R data exploration and visualisation}.
...
\item
  ...
  We doen dit steeds via de aesthetics \texttt{aes} functie. \texttt{aes(x=trt,y=mean)}
\item
...
\item
  We voegen foutenvlaggen toe om de onzekerheid op het gemiddelde weer te geven. We doen dit via de \texttt{geom\_errorbar} functie en specifiëren het minimum en maximum van de error bar. Het \texttt{width} argument wordt gebruikt om de breedte van de error bar smaller te maken dan deze van de bar.
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{apRelSum }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ trt, }\AttributeTok{y =}\NormalTok{ mean)) }\SpecialCharTok{+} \FunctionTok{geom\_bar}\NormalTok{(}\AttributeTok{stat =} \StringTok{"identity"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_errorbar}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{ymin =}\NormalTok{ mean }\SpecialCharTok{{-}}\NormalTok{ sd, }\AttributeTok{ymax =}\NormalTok{ mean }\SpecialCharTok{+}
\NormalTok{        sd), }\AttributeTok{width =} \FloatTok{0.2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-7-1.pdf}}
...
\NormalTok{ap }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ trt, }\AttributeTok{y =}\NormalTok{ rel)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-8-1.pdf}}
...
\begin{Highlighting}[]
\NormalTok{ap }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ trt, }\AttributeTok{y =}\NormalTok{ rel)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-9-1.pdf}}
...
Gemiddeld is de relatieve abundantie van Staphylococcus in het microbiome van personen met een zweetgeur is 22.2\% hoger na microbiome transplantatie dan na de placebo behandeling.
...
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{facet\_grid}\NormalTok{(Gender }\SpecialCharTok{\textasciitilde{}}
\NormalTok{    .) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Lengte (cm)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-12-1.pdf}}
...
\NormalTok{HeightSum }\OtherTok{\textless{}{-}}\NormalTok{ nhanesSub }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{summarize\_at}\NormalTok{(}\StringTok{"Height"}\NormalTok{, }\FunctionTok{list}\NormalTok{(}\AttributeTok{mean =}\NormalTok{ mean, }\AttributeTok{sd =}\NormalTok{ sd))}

\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(HeightSum }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate\_if}\NormalTok{(is.numeric, round, }\AttributeTok{digits =} \DecValTok{1}\NormalTok{), tableType)}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-16-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-17-1.pdf}}
...
\NormalTok{samp2 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Gender, }\AttributeTok{y =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \DecValTok{1}\NormalTok{,}
    \AttributeTok{y =}\NormalTok{ HeightSumExp2}\SpecialCharTok{$}\NormalTok{mean[}\DecValTok{1}\NormalTok{]), }\AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{pch =} \DecValTok{17}\NormalTok{,}
    \AttributeTok{color =} \StringTok{"darkred"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \DecValTok{2}\NormalTok{, }\AttributeTok{y =}\NormalTok{ HeightSumExp2}\SpecialCharTok{$}\NormalTok{mean[}\DecValTok{2}\NormalTok{]),}
    \AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{pch =} \DecValTok{17}\NormalTok{, }\AttributeTok{color =} \StringTok{"darkred"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Height (cm)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-19-1.pdf}}
...
\NormalTok{samp3 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Gender, }\AttributeTok{y =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \DecValTok{1}\NormalTok{,}
    \AttributeTok{y =}\NormalTok{ HeightSumExp3}\SpecialCharTok{$}\NormalTok{mean[}\DecValTok{1}\NormalTok{]), }\AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{pch =} \DecValTok{17}\NormalTok{,}
    \AttributeTok{color =} \StringTok{"darkred"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \DecValTok{2}\NormalTok{, }\AttributeTok{y =}\NormalTok{ HeightSumExp3}\SpecialCharTok{$}\NormalTok{mean[}\DecValTok{2}\NormalTok{]),}
    \AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{pch =} \DecValTok{17}\NormalTok{, }\AttributeTok{color =} \StringTok{"darkred"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Height (cm)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-20-1.pdf}}
...
\CommentTok{\# Aantal simulaties en steekproefgrootte per}
\CommentTok{\# groep}
...
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{verschil =} \FunctionTok{colMeans}\NormalTok{(femSamps) }\SpecialCharTok{{-}} \FunctionTok{colMeans}\NormalTok{(malSamps),}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ verschil, }\AttributeTok{y =} \SpecialCharTok{{-}}\FunctionTok{log10}\NormalTok{(pvalue), }\AttributeTok{color =}\NormalTok{ pvalue }\SpecialCharTok{\textless{}}
        \FloatTok{0.05}\NormalTok{)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Gemiddeld Verschil (cm)"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(}\StringTok{"Statistische Significantie ({-}log10 p)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-21-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ verschil)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Gemiddeld Verschil (cm)"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{xlab}\NormalTok{(}\StringTok{""}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-21-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-22-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-22-2.pdf}}
...
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{verschil =} \FunctionTok{colMeans}\NormalTok{(femSamps) }\SpecialCharTok{{-}} \FunctionTok{colMeans}\NormalTok{(femSamps2),}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-23-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-23-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-24-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-24-2.pdf}}
...
\item
  Met statistiek controleren we de kans op het trekken van foute conclusies.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{nfip }\OtherTok{\textless{}{-}} \FunctionTok{tibble}\NormalTok{(}\AttributeTok{group =} \FunctionTok{c}\NormalTok{(}\StringTok{"cases"}\NormalTok{, }\StringTok{"controls"}\NormalTok{, }\StringTok{"noConcent"}\NormalTok{),}
    \AttributeTok{grade =} \FunctionTok{c}\NormalTok{(}\StringTok{"g2"}\NormalTok{, }\StringTok{"g1g3"}\NormalTok{, }\StringTok{"g2"}\NormalTok{), }\AttributeTok{vaccin =} \FunctionTok{c}\NormalTok{(}\StringTok{"yes"}\NormalTok{,}
        \StringTok{"no"}\NormalTok{, }\StringTok{"no"}\NormalTok{), }\AttributeTok{total =} \FunctionTok{c}\NormalTok{(}\DecValTok{221998}\NormalTok{, }\DecValTok{725173}\NormalTok{, }\DecValTok{123605}\NormalTok{),}
    \AttributeTok{polio =} \FunctionTok{c}\NormalTok{(}\DecValTok{54}\NormalTok{, }\DecValTok{391}\NormalTok{, }\DecValTok{56}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
...
\NormalTok{nfip }\OtherTok{\textless{}{-}}\NormalTok{ nfip }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{incidencePM =} \FunctionTok{round}\NormalTok{(nfip}\SpecialCharTok{$}\NormalTok{polio}\SpecialCharTok{/}\NormalTok{nfip}\SpecialCharTok{$}\NormalTok{total }\SpecialCharTok{*}
        \FloatTok{1e+06}\NormalTok{, }\DecValTok{0}\NormalTok{))}
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(nfip, tableType)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.5\linewidth]{Statistiek_2026_2027_files/figure-latex/unnamed-chunk-27-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{salk }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{group =} \FunctionTok{c}\NormalTok{(}\StringTok{"cases"}\NormalTok{, }\StringTok{"control"}\NormalTok{, }\StringTok{"noConcent"}\NormalTok{),}
    \AttributeTok{treatment =} \FunctionTok{c}\NormalTok{(}\StringTok{"vaccine"}\NormalTok{, }\StringTok{"placebo"}\NormalTok{, }\StringTok{"none"}\NormalTok{), }\AttributeTok{total =} \FunctionTok{c}\NormalTok{(}\DecValTok{200745}\NormalTok{,}
        \DecValTok{201229}\NormalTok{, }\DecValTok{338778}\NormalTok{), }\AttributeTok{polio =} \FunctionTok{c}\NormalTok{(}\DecValTok{57}\NormalTok{, }\DecValTok{142}\NormalTok{, }\DecValTok{157}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{noPolio =}\NormalTok{ total }\SpecialCharTok{{-}}\NormalTok{ polio, }\AttributeTok{incidencePM =} \FunctionTok{round}\NormalTok{(polio}\SpecialCharTok{/}\NormalTok{total }\SpecialCharTok{*}
        \FloatTok{1e+06}\NormalTok{, }\DecValTok{0}\NormalTok{))}
...
\begin{itemize}
\item
  We observeren een veel groter effect nu cases en controles vergelijkbaar zijn, incidentie van respectievelijk 284 en 706 per miljoen.
\item
  De polio incidentie voor kinderen die geen toestemming geven blijft vergelijkbaar 453 en 463 per miljoen respectievelijk in de NFIP- en Salk-studie.
\end{itemize}
...
Link naar webpage/script die wordt gebruikt in de kennisclips: \href{https://statomics.github.io/sbc/rmd/02-concepts.html}{script Hoofdstuk2}
...
\emph{Proefopzet} of \emph{studie design} is een aparte tak van de statistiek en is een cruciaal onderdeel van een studie.
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/pop2Samp2Pop-1} 
...
onderverdeeld in \emph{nominale variabelen} en \emph{ordinale variabelen}. Nominale gegevens zijn gegevens die men kan benoemen. Ze worden niet gemeten en kennen geen natuurlijke ordening; bijvoorbeeld geslacht, ras,
...
\item
  \(Pr(\Omega)=1\).
\item
  Voor mutueel exclusieve events \(A_1, A_2, \ldots A_k\) geldt dat \(Pr(A_1 \cup A_2 \ldots \cup A_k)= Pr(A_1) + \ldots + Pr(A_k)\).
\end{enumerate}
...
\FunctionTok{tibble}\NormalTok{(}\AttributeTok{X =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{), }\AttributeTok{prob =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.508}\NormalTok{, }\FloatTok{0.492}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{geom\_segment}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Probability"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-31-1.pdf}}

Toevallig veranderlijke \(X\) volgt een Bernoulli verdeling \(B(\pi)\) met parameter \(\pi=0.492\),
\[B(\pi)= \pi^x(1-\pi)^{(1-x)}\]

\paragraph{Cumulatieve distributie functie}\label{cumulatieve-distributie-functie}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{tibble}\NormalTok{(}\AttributeTok{X =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{0.5}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\FloatTok{1.5}\NormalTok{), }\AttributeTok{cumprob =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.508}\NormalTok{,}
    \DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ X, }\AttributeTok{y =}\NormalTok{ cumprob)) }\SpecialCharTok{+} \FunctionTok{geom\_step}\NormalTok{(}\AttributeTok{direction =} \StringTok{"hv"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(}\StringTok{"F(x)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-32-1.pdf}}

Dobbelsteen voorbeeld:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{tibble}\NormalTok{(}\AttributeTok{X =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{6}\NormalTok{, }\FloatTok{6.5}\NormalTok{), }\AttributeTok{cumprob =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FunctionTok{cumsum}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\DecValTok{1}\SpecialCharTok{/}\DecValTok{6}\NormalTok{,}
    \DecValTok{6}\NormalTok{)), }\DecValTok{1}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ X, }\AttributeTok{y =}\NormalTok{ cumprob)) }\SpecialCharTok{+} \FunctionTok{geom\_step}\NormalTok{(}\AttributeTok{direction =} \StringTok{"hv"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(}\StringTok{"F(x)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-33-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{iq }\OtherTok{\textless{}{-}} \FunctionTok{tibble}\NormalTok{(}\AttributeTok{IQ =} \FunctionTok{seq}\NormalTok{(}\DecValTok{40}\NormalTok{, }\DecValTok{150}\NormalTok{, }\FloatTok{0.1}\NormalTok{), }\AttributeTok{Densiteit =} \FunctionTok{dnorm}\NormalTok{(}\FunctionTok{seq}\NormalTok{(}\DecValTok{40}\NormalTok{,}
    \DecValTok{150}\NormalTok{, }\FloatTok{0.1}\NormalTok{), }\AttributeTok{mean =} \DecValTok{100}\NormalTok{, }\AttributeTok{sd =} \DecValTok{15}\NormalTok{))}
\NormalTok{iq }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ IQ, }\AttributeTok{y =}\NormalTok{ Densiteit)) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics{Statistiek_2026_2027_files/figure-latex/IQ-1} \end{center}
...
\NormalTok{iq }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ IQ, }\AttributeTok{y =}\NormalTok{ Probability)) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-34-1.pdf}}
...
\begin{center}\includegraphics{Statistiek_2026_2027_files/figure-latex/unnamed-chunk-36-1} \end{center}

\begin{center}\includegraphics{Statistiek_2026_2027_files/figure-latex/unnamed-chunk-37-1} \end{center}
...
\begin{center}\includegraphics{Statistiek_2026_2027_files/figure-latex/unnamed-chunk-38-1} \end{center}
...
\begin{center}\includegraphics{Statistiek_2026_2027_files/figure-latex/unnamed-chunk-39-1} \end{center}
...
We kunnen de \texttt{qnorm} functie gebruiken om kwantielen \(z_{2.5\%}\) en \(z_{97.5\%}\) te berekenen die respectievelijk corresponderen met \(F(z_{2.5\%})=0.025\) en \(F(z_{97.5\%})=0.975\).
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/continu-1} 

}

\caption{Bovenste rij: dichtheidsfunctie van een Normale verdeling met gemiddelde $\mu=80$ en standaardafwijking $\sigma=12$, met de rode oppervlakte onder de curve die links de kans $P(X<64)$, in het midden de kans $P(X<80)$ en rechts de kans $P(64<X<80)$ voorstelt. Onderste rij: overeenkomstige distributiefunctie $F(x)=P(X\leq x)$, met telkens de waarde van $x$ en de bijhorende kans aangeduid.}\label{fig:continu}
\end{figure}
...
voor \(\mu=80\) en \(\sigma=12\) en wordt een \emph{distributiefunctie} genoemd.
...
\FunctionTok{library}\NormalTok{(NHANES)}
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Gender)) }\SpecialCharTok{+} \FunctionTok{geom\_bar}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-45-1.pdf}}
...
\begin{Highlighting}[]
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Gender }\SpecialCharTok{==} \StringTok{"female"} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(Height) }\SpecialCharTok{\&}\NormalTok{ Age }\SpecialCharTok{\textgreater{}}
        \DecValTok{18}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-47-1.pdf}}
...
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Gender }\SpecialCharTok{==} \StringTok{"female"} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(Height) }\SpecialCharTok{\&}\NormalTok{ Age }\SpecialCharTok{\textgreater{}}
        \DecValTok{18}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{stat\_ecdf}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-48-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-50-1.pdf}}
...
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Gender }\SpecialCharTok{==} \StringTok{"female"} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(Height) }\SpecialCharTok{\&}\NormalTok{ Age }\SpecialCharTok{\textgreater{}}
        \DecValTok{18}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..)) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Lengte (cm)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dnorm,}
    \AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{mean =}\NormalTok{ HeightSum}\SpecialCharTok{$}\NormalTok{mean[}\DecValTok{1}\NormalTok{],}
        \AttributeTok{sd =}\NormalTok{ HeightSum}\SpecialCharTok{$}\NormalTok{sd[}\DecValTok{1}\NormalTok{]))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-55-1.pdf}}
...
\NormalTok{fem10 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..), }\AttributeTok{bins =} \DecValTok{10}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Lengte (cm)"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dnorm, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{mean =}\NormalTok{ HeightSum10}\SpecialCharTok{$}\NormalTok{mean[}\DecValTok{1}\NormalTok{],}
        \AttributeTok{sd =}\NormalTok{ HeightSum10}\SpecialCharTok{$}\NormalTok{sd[}\DecValTok{1}\NormalTok{])) }\SpecialCharTok{+} \FunctionTok{xlim}\NormalTok{(}\DecValTok{130}\NormalTok{, }\DecValTok{190}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-57-1.pdf}}
...
\textbf{Belangrijke Conventie:} In de cursus gebruiken we de conventie om \textbf{populatieparameters die een vaste waarde aannemen maar die meestal ongekend zijn} voor te stellen door \textbf{Griekse symbolen}. \textbf{Statistieken} waarmee we deze ongekende parameters schatten o.b.v. een steekproef zullen we weergeven door \textbf{letters}.
...
Link naar webpage/script die wordt gebruik in de kennisclips: \href{https://statomics.github.io/sbc/rmd/03-experimentalDesign.html}{script Hoofdstuk3}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/pop2Samp2PopDesign-1} 
...
\footnote{Met ``zuiver'' wordt hier bedoeld dat het zuivere interventie-effect uit de gegevens
  kan gehaald worden zonder dat het antwoord wordt beïnvloed door andere aspecten/variabelen.}
...
behandelingseffect vertekend\footnote{ Voorlopig verstaan we onder het feit dat een schatting voor het
  behandelingseffect `vertekend' is, dat het foutief werd ingeschat of, m.a.w., dat het geschatte effect niet correct het zuivere effect van de behandeling weerspiegelt.} is door vergelijkbare groepen dieren te creëren;
...
van het vaccin buiten het laboratorium na te gaan. Meer concreet wenste men
...
725000 kinderen uit de eerste en derde graad werden vrijwillig niet
...
zonder P. glutinosus te corrigeren voor de confounder voedselrijkdom, kan men proberen een index te verzamelen voor de voedselrijkdom van elk gebied en vervolgens de analyse
afzonderlijk uitvoeren bij gebieden met dezelfde voedselrijkdom.
Men zegt dan dat de analyse of het geschatte
effect van P. glutinosus op de groei van P. jordani \emph{gecontroleerd} (in het Engels:
...
jongens, behalve in richtingen C en E waar de jongens het lichtjes beter doen. Dit
...
Merk op dat, hoewel onder de mensen die het allel Leu/Leu bezitten \(89/145=61.4\%\) aan borstkanker lijdt, dit cijfer niet
...
Link naar webpage/script die wordt gebruik in de kennisclips: \href{https://statomics.github.io/sbc/rmd/04-dataExploration.html}{script Hoofdstuk4}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/pop2Samp2PopDataExpl-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# De data van de NHANES studie bevindt zich in}
\CommentTok{\# het R package NHANES}
\FunctionTok{library}\NormalTok{(NHANES)  }\CommentTok{\#laad NHANES package}

\CommentTok{\# NHANES is een data frame met de gegevens De}
\CommentTok{\# rijen bevatten informatie over elk subject De}
\CommentTok{\# kolommen de variabelen die werden geregistreerd}
\CommentTok{\# vb variabele Gender, BMI, ... Een variabele}
\CommentTok{\# (kolom) kan uit de dataframe worden gehaald}
\CommentTok{\# door gebruik van het $ teken en de naam van de}
\CommentTok{\# variabele We slaan de frequentietabel voor}
\CommentTok{\# variable Gender op in object \textquotesingle{}tab\textquotesingle{}}
...
\begin{Highlighting}[]
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Gender)) }\SpecialCharTok{+} \FunctionTok{geom\_bar}\NormalTok{(}\AttributeTok{fill =} \StringTok{"steelblue"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-88-1.pdf}}
...
\begin{Highlighting}[]
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ BMI)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..), }\AttributeTok{bins =} \DecValTok{30}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_density}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/histo-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{quantile}\NormalTok{(NHANES}\SpecialCharTok{$}\NormalTok{BMI, }\FunctionTok{c}\NormalTok{(}\FloatTok{0.25}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\FloatTok{0.75}\NormalTok{), }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/boxBMI-1} 
...
\begin{Highlighting}[]
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \StringTok{""}\NormalTok{, }\AttributeTok{y =}\NormalTok{ BMI)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-92-1.pdf}}
...
\emph{Indien men de grootste observatie (81.25) vervangt door 8125 om als het ware een tikfout voor te stellen, dan wijzigt het rekenkundig gemiddelde naar 27.5 en dat terwijl er bijna 10000 BMI metingen zijn. Merk op dat het gemiddelde vrij sterk beïnvloed kan worden door één outlier.}
...
\CommentTok{\# ontbrekende waarnemingen Die worden in R als}
\CommentTok{\# een NA voorgesteld Als we het argument}
\CommentTok{\# na.rm=TRUE gebruiken wordt de mediaan berekend}
\CommentTok{\# op basis van de beschikbare observaties}
\end{Highlighting}
\end{Shaded}

Indien men de grootste observatie (81.25) vervangt door 8125, dan wijzigt de mediaan niet.
...
Het \textbf{geometrische gemiddelde} van een reeks waarnemingen \(x_i, i=1,
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{logSummary }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{summarize}\NormalTok{(}\AttributeTok{logMean =} \FunctionTok{mean}\NormalTok{(BMI }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{(), }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{), }\AttributeTok{sd =} \FunctionTok{sd}\NormalTok{(BMI }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{(), }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{), }\AttributeTok{mean =} \FunctionTok{mean}\NormalTok{(BMI, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
        \AttributeTok{median =} \FunctionTok{median}\NormalTok{(BMI, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{geoMean =} \DecValTok{2}\SpecialCharTok{\^{}}\NormalTok{logMean)}

\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ BMI }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..), }\AttributeTok{bins =} \DecValTok{30}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_density}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dnorm, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{mean =}\NormalTok{ logSummary}\SpecialCharTok{$}\NormalTok{logMean,}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/histLogBMI-1} 
...
Het gemiddelde en mediane BMI bedraagt 26.66 en 25.98 , respectievelijk. Het gemiddelde is hier groter dan de mediaan omdat de BMI scheef verdeeld is naar rechts (zie Figuur \ref{fig:histLogBMI}). De verdeling wordt meer symmetrisch na log-transformatie. Het gemiddelde en mediane log-BMI liggen ook dichter bij elkaar en bedragen respectievelijk 3.25 en 3.26. De geometrische gemiddelde BMI-concentratie bekomen we door de exponentiële functie te evalueren in 3.25, hetgeen ons 25.69 oplevert. Merk op dat dit inderdaad beter met de mediaan overeenstemt dan het rekenkundig gemiddelde.
...
\NormalTok{normSim }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{(}\FunctionTok{rnorm}\NormalTok{(n }\SpecialCharTok{*}\NormalTok{ nSamp, }\AttributeTok{mean =}\NormalTok{ mu, }\AttributeTok{sd =}\NormalTok{ sigma),}
    \AttributeTok{nrow =}\NormalTok{ n) }\SpecialCharTok{|\textgreater{}}
...
\NormalTok{normSim }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ data)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..), }\AttributeTok{bins =} \DecValTok{30}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_density}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..)) }\SpecialCharTok{+}
    \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{samp)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-105-1.pdf}}
...
\NormalTok{normSim }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{gather}\NormalTok{(samp, data) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ samp, }\AttributeTok{y =}\NormalTok{ data)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-106-1.pdf}}
...
\NormalTok{normSim }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{gather}\NormalTok{(samp, data) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ data)) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{samp)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-107-1.pdf}}
...
\NormalTok{p1 }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Gender }\SpecialCharTok{==} \StringTok{"female"} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(BMI)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ BMI)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..)) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"BMI"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"All females in study"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_density}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..))}

\NormalTok{p2 }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Gender }\SpecialCharTok{==} \StringTok{"female"} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(BMI)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ BMI)) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{()}

\FunctionTok{grid.arrange}\NormalTok{(p1, p2, }\AttributeTok{ncol =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-108-1.pdf}}
...
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"log(Direct Cholestorol)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/boxplotCholGender-1} 
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-114-1.pdf}}
...
\NormalTok{captopril }\OtherTok{\textless{}{-}} \FunctionTok{read.table}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/statOmics/sbc/master/data/captopril.txt"}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{captoprilTidy }\OtherTok{\textless{}{-}}\NormalTok{ captopril }\SpecialCharTok{|\textgreater{}}
...
\NormalTok{captoprilTidy }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ type, }\AttributeTok{y =}\NormalTok{ bp)) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{group =}\NormalTok{ id)) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoDot-1} 
...
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Height, }\AttributeTok{y =}\NormalTok{ Weight)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-115-1.pdf}}
...
\NormalTok{p1 }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Age }\SpecialCharTok{\textgreater{}=} \DecValTok{18} \SpecialCharTok{\&}\NormalTok{ Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..)) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Height"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"All females in study"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_density}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..))}

\NormalTok{p2 }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Age }\SpecialCharTok{\textgreater{}=} \DecValTok{18} \SpecialCharTok{\&}\NormalTok{ Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ Height)) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{()}
\FunctionTok{grid.arrange}\NormalTok{(p1, p2, }\AttributeTok{ncol =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-116-1.pdf}}

De lengte data zijn duidelijk approximatief normaal verdeeld.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p3 }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Age }\SpecialCharTok{\textgreater{}=} \DecValTok{18} \SpecialCharTok{\&}\NormalTok{ Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Weight)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..)) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Weight"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"All females in study"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_density}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..))}

\NormalTok{p4 }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Age }\SpecialCharTok{\textgreater{}=} \DecValTok{18} \SpecialCharTok{\&}\NormalTok{ Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ Weight)) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{()}

\FunctionTok{grid.arrange}\NormalTok{(p3, p4, }\AttributeTok{ncol =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-117-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p5 }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Age }\SpecialCharTok{\textgreater{}=} \DecValTok{18} \SpecialCharTok{\&}\NormalTok{ Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Weight }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..)) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Weight (log2)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"All females in study"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_density}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..))}

\NormalTok{p6 }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Age }\SpecialCharTok{\textgreater{}=} \DecValTok{18} \SpecialCharTok{\&}\NormalTok{ Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ Weight }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{()}

\FunctionTok{grid.arrange}\NormalTok{(p5, p6, }\AttributeTok{ncol =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-118-1.pdf}}
...
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Age }\SpecialCharTok{\textgreater{}=} \DecValTok{18} \SpecialCharTok{\&}\NormalTok{ Gender }\SpecialCharTok{==} \StringTok{"female"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Height, }\AttributeTok{y =}\NormalTok{ Weight }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Weight (log2)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-119-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:bijdrageCor}Bijdrage van individuele metingen in de correlatie.}]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/bijdrageCor-1.pdf}}
...
\NormalTok{NHANES }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{log2Weight =}\NormalTok{ Weight }\SpecialCharTok{|\textgreater{}}
...
\NormalTok{p1 }\OtherTok{\textless{}{-}}\NormalTok{ simData }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ y)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\StringTok{"cor ="}\NormalTok{,}
    \FunctionTok{cor}\NormalTok{(simData[, }\DecValTok{1}\NormalTok{], simData[, }\DecValTok{2}\NormalTok{]) }\SpecialCharTok{|\textgreater{}}
...
\NormalTok{p2 }\OtherTok{\textless{}{-}}\NormalTok{ outlier }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ y)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\StringTok{"cor ="}\NormalTok{,}
    \FunctionTok{cor}\NormalTok{(outlier[, }\DecValTok{1}\NormalTok{], outlier[, }\DecValTok{2}\NormalTok{]) }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{round}\NormalTok{(}\DecValTok{2}\NormalTok{)))}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:corOutlier}Correlatie van gesimuleerde data met 1 outlier}]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/corOutlier-1.pdf}}
...
\NormalTok{quadratic }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ y)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\StringTok{"cor ="}\NormalTok{,}
    \FunctionTok{cor}\NormalTok{(quadratic[, }\DecValTok{1}\NormalTok{], quadratic[, }\DecValTok{2}\NormalTok{]) }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{round}\NormalTok{(}\DecValTok{2}\NormalTok{))) }\SpecialCharTok{+} \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =} \FunctionTok{mean}\NormalTok{(quadratic[,}
    \DecValTok{2}\NormalTok{]), }\AttributeTok{col =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =} \FunctionTok{mean}\NormalTok{(quadratic[,}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:corQuad}Correlatie van gesimuleerde data met een kwadratisch verband. De data in de bovenste kwadranten compenseren elkaar als het ware alsook in de onderste kwadranten (ongeveer evenveel positieve en negatieve bijdragen door de data in de correlatie)}]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/corQuad-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-121-1.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-121-2.pdf}}
...
\begin{itemize}
\item
  \href{https://statomics.github.io/sbc/rmd/05-statisticalInference.html}{script Hoofdstuk5}
\item
  \href{https://statomics.github.io/sbc/rmd/05-statisticalInference-twosampleT.html}{script Hoofdstuk5: Two-sample t-test}
\end{itemize}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoPop2Samp2Pop-1} 
...
De data is beschikbaar in een tekstbestand met naam \texttt{captopril.txt} op de github pagina \url{https://raw.githubusercontent.com/statOmics/sbc/master/data/captopril.txt}.
...
\NormalTok{captoprilTidy }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{summarize\_at}\NormalTok{(}\StringTok{"bp"}\NormalTok{, }\FunctionTok{list}\NormalTok{(}\AttributeTok{mean =} \SpecialCharTok{\textasciitilde{}}\FunctionTok{mean}\NormalTok{(., }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
        \AttributeTok{sd =} \SpecialCharTok{\textasciitilde{}}\FunctionTok{sd}\NormalTok{(., }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{), }\AttributeTok{n =} \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{sum}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(x)))) }\SpecialCharTok{|\textgreater{}}
...
\NormalTok{captoprilTidy }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ type, }\AttributeTok{y =}\NormalTok{ mean)) }\SpecialCharTok{+} \FunctionTok{geom\_bar}\NormalTok{(}\AttributeTok{stat =} \StringTok{"identity"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_errorbar}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{ymin =}\NormalTok{ mean }\SpecialCharTok{{-}}\NormalTok{ se, }\AttributeTok{ymax =}\NormalTok{ mean }\SpecialCharTok{+}
\NormalTok{        se), }\AttributeTok{width =} \FloatTok{0.2}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylim}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{210}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"blood pressure (mmHg)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoBar-1} 
...
\NormalTok{captoprilTidy }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ type, }\AttributeTok{y =}\NormalTok{ bp)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylim}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{210}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoBox-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# De geom\_line layer laat ons de}
\CommentTok{\# bloeddrukmetingen voor dezelfde personen}
\CommentTok{\# verbinden met een lijn}
\NormalTok{captoprilTidy }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(type }\SpecialCharTok{\%in\%} \FunctionTok{c}\NormalTok{(}\StringTok{"SBPa"}\NormalTok{, }\StringTok{"SBPb"}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{type =} \FunctionTok{factor}\NormalTok{(type, }\AttributeTok{levels =} \FunctionTok{c}\NormalTok{(}\StringTok{"SBPb"}\NormalTok{, }\StringTok{"SBPa"}\NormalTok{))) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ type, }\AttributeTok{y =}\NormalTok{ bp)) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{group =}\NormalTok{ id)) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoDotBsl-1} 
...
\NormalTok{captopril }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(}\StringTok{"Difference (mm mercury)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{""}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoBoxDelta-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoBoxDiffQQ-1} 
...
\NormalTok{        p }\OtherTok{\textless{}{-}}\NormalTok{ femSamp }\SpecialCharTok{|\textgreater{}}
            ...
            \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{stat\_summary}\NormalTok{(}\AttributeTok{fun.y =}\NormalTok{ mean,}
            \AttributeTok{geom =} \StringTok{"point"}\NormalTok{, }\AttributeTok{shape =} \DecValTok{19}\NormalTok{, }\AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{,}
            \AttributeTok{fill =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =} \FunctionTok{mean}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
            \FunctionTok{log2}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"cholesterol (log2)"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-129-1.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-129-2.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-129-3.pdf}}
...
\NormalTok{femSamp }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{gather}\NormalTok{(}\StringTok{"sample"}\NormalTok{, }\StringTok{"log2cholesterol"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ sample, }\AttributeTok{y =}\NormalTok{ log2cholesterol)) }\SpecialCharTok{+}
    \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{stat\_summary}\NormalTok{(}\AttributeTok{fun.y =}\NormalTok{ mean, }\AttributeTok{geom =} \StringTok{"point"}\NormalTok{,}
    \AttributeTok{shape =} \DecValTok{19}\NormalTok{, }\AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =} \FunctionTok{mean}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"cholesterol (log2)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-129-4.pdf}}
...
\NormalTok{femSamp10 }\OtherTok{\textless{}{-}} \FunctionTok{sapply}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{1000}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(j, x, size) }\FunctionTok{sample}\NormalTok{(x,}
\NormalTok{    size), }\AttributeTok{size =} \DecValTok{10}\NormalTok{, }\AttributeTok{x =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol)}

\NormalTok{femSamp50 }\OtherTok{\textless{}{-}} \FunctionTok{sapply}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{1000}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(j, x, size) }\FunctionTok{sample}\NormalTok{(x,}
\NormalTok{    size), }\AttributeTok{size =} \DecValTok{50}\NormalTok{, }\AttributeTok{x =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol)}

\NormalTok{femSamp100 }\OtherTok{\textless{}{-}} \FunctionTok{sapply}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{1000}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(j, x, size) }\FunctionTok{sample}\NormalTok{(x,}
\NormalTok{    size), }\AttributeTok{size =} \DecValTok{100}\NormalTok{, }\AttributeTok{x =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol)}

\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{rbind}\NormalTok{(femSamp10 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{summarize\_at}\NormalTok{(}\StringTok{"log2Chol"}\NormalTok{, }\FunctionTok{list}\NormalTok{(}\AttributeTok{median =} \SpecialCharTok{\textasciitilde{}}\FunctionTok{median}\NormalTok{(.,}
        \AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{), }\AttributeTok{mean =} \SpecialCharTok{\textasciitilde{}}\FunctionTok{mean}\NormalTok{(., }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
        \AttributeTok{sd =} \SpecialCharTok{\textasciitilde{}}\FunctionTok{sd}\NormalTok{(., }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{), }\AttributeTok{n =} \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{sum}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(x)))) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{se =}\NormalTok{ sd}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(n)), femSamp50 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{gather}\NormalTok{(sample, log2Chol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{group\_by}\NormalTok{(sample) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{summarize\_at}\NormalTok{(}\StringTok{"log2Chol"}\NormalTok{, }\FunctionTok{list}\NormalTok{(}\AttributeTok{median =} \SpecialCharTok{\textasciitilde{}}\FunctionTok{median}\NormalTok{(.,}
        \AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{), }\AttributeTok{mean =} \SpecialCharTok{\textasciitilde{}}\FunctionTok{mean}\NormalTok{(., }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
        \AttributeTok{sd =} \SpecialCharTok{\textasciitilde{}}\FunctionTok{sd}\NormalTok{(., }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{), }\AttributeTok{n =} \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{sum}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(x)))) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{se =}\NormalTok{ sd}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(n)), femSamp100 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{gather}\NormalTok{(sample, log2Chol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{group\_by}\NormalTok{(sample) }\SpecialCharTok{|\textgreater{}}
...
\begin{Highlighting}[]
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ n }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{as.factor}\NormalTok{(), }\AttributeTok{y =}\NormalTok{ mean)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(}\StringTok{"Direct cholesterol (log2)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"sample size"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-134-1.pdf}}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ n }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{as.factor}\NormalTok{(), }\AttributeTok{y =}\NormalTok{ sd)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"standard deviation"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{xlab}\NormalTok{(}\StringTok{"sample size"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-135-1.pdf}}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ n }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{as.factor}\NormalTok{(), }\AttributeTok{y =}\NormalTok{ se)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"standard error"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{xlab}\NormalTok{(}\StringTok{"sample size"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-136-1.pdf}}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ type, }\AttributeTok{y =}\NormalTok{ estimate)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{mean}\NormalTok{()) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"10 personen"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-137-1.pdf}}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{mean}\NormalTok{()) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"50 personen"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-138-1.pdf}}
...
\NormalTok{fem }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ DirectChol }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..)) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Direct cholesterol (log2)"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dnorm, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{mean =} \FunctionTok{mean}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{()), }\AttributeTok{sd =} \FunctionTok{sd}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{()))) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Alle vrouwen in NHANES studie"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-139-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fem }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ DirectChol }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{stat\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{stat\_qq\_line}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Alle vrouwen in NHANES study"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-139-2.pdf}}
...
\FunctionTok{data.frame}\NormalTok{(log2Chol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2Chol)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..), }\AttributeTok{bins =} \DecValTok{10}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Direct cholesterol (log2)"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dnorm, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{mean =}\NormalTok{ femSamp10[,}
        \DecValTok{1}\NormalTok{] }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{mean}\NormalTok{(), }\AttributeTok{sd =}\NormalTok{ femSamp10[, }\DecValTok{1}\NormalTok{] }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{sd}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"10 random females"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlim}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{range}\NormalTok{())}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-140-1.pdf}}
...
\FunctionTok{data.frame}\NormalTok{(meanLog2Chol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ meanLog2Chol)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..), }\AttributeTok{bins =} \DecValTok{15}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Mean cholesterol (log2)"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dnorm, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{mean =}\NormalTok{ femSamp10 }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{colMeans}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{mean}\NormalTok{(), }\AttributeTok{sd =}\NormalTok{ femSamp10 }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{colMeans}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{sd}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Means on 10 females"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-141-1.pdf}}
...
\NormalTok{meanLog2Chol }\OtherTok{\textless{}{-}}\NormalTok{ femSamp10 }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{colMeans}\NormalTok{()}
\FunctionTok{data.frame}\NormalTok{(meanLog2Chol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ meanLog2Chol)) }\SpecialCharTok{+} \FunctionTok{stat\_qq}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{stat\_qq\_line}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Means on 10 females"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-141-2.pdf}}
...
\begin{Highlighting}[]
\NormalTok{fem }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ DirectChol)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..)) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Direct cholesterol"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dnorm, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{mean =} \FunctionTok{mean}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol),}
        \AttributeTok{sd =} \FunctionTok{sd}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol))) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"All females in Nhanes study"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-142-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fem }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ DirectChol)) }\SpecialCharTok{+} \FunctionTok{stat\_qq}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{stat\_qq\_line}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"All females in Nhanes study"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-142-2.pdf}}
...
\NormalTok{femSamp5 }\OtherTok{\textless{}{-}} \FunctionTok{sapply}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{1000}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(j, x, size) }\FunctionTok{sample}\NormalTok{(x,}
\NormalTok{    size), }\AttributeTok{size =} \DecValTok{5}\NormalTok{, }\AttributeTok{x =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol)}
...
\FunctionTok{data.frame}\NormalTok{(meanChol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ meanChol)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..), }\AttributeTok{bins =} \DecValTok{15}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Mean cholesterol"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dnorm, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{mean =}\NormalTok{ femSamp5 }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{colMeans}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{mean}\NormalTok{(), }\AttributeTok{sd =}\NormalTok{ femSamp5 }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{colMeans}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{sd}\NormalTok{())) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Means on 5 females"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-143-1.pdf}}
...
\NormalTok{meanChol }\OtherTok{\textless{}{-}}\NormalTok{ femSamp5 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{colMeans}\NormalTok{()}
\FunctionTok{data.frame}\NormalTok{(meanChol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ meanChol)) }\SpecialCharTok{+} \FunctionTok{stat\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{stat\_qq\_line}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Means on 5 females"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-143-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-143-3.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-143-4.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-143-5.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-143-6.pdf}}
...
\begin{definition}[95 procent betrouwbaarheidsinterval voor populatiegemiddelde]
...
\NormalTok{ll }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(samp50 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{()) }\SpecialCharTok{{-}} \FloatTok{1.96} \SpecialCharTok{*} \FunctionTok{sd}\NormalTok{(samp50 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{())}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(}\DecValTok{50}\NormalTok{)}
\NormalTok{ul }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(samp50 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{()) }\SpecialCharTok{+} \FloatTok{1.96} \SpecialCharTok{*} \FunctionTok{sd}\NormalTok{(samp50 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{())}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(}\DecValTok{50}\NormalTok{)}
\NormalTok{popMean }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{())}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res}\SpecialCharTok{$}\NormalTok{ll }\OtherTok{\textless{}{-}}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{mean }\SpecialCharTok{{-}} \FloatTok{1.96} \SpecialCharTok{*}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{se}
\NormalTok{res}\SpecialCharTok{$}\NormalTok{ul }\OtherTok{\textless{}{-}}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{mean }\SpecialCharTok{+} \FloatTok{1.96} \SpecialCharTok{*}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{se}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_errorbar}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{ymin =}\NormalTok{ mean }\SpecialCharTok{{-}}
    \FloatTok{1.96} \SpecialCharTok{*}\NormalTok{ se, }\AttributeTok{ymax =}\NormalTok{ mean }\SpecialCharTok{+} \FloatTok{1.96} \SpecialCharTok{*}\NormalTok{ se)) }\SpecialCharTok{+} \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mean}\NormalTok{()) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"20 CI for N = 10"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylim}\NormalTok{(}\FunctionTok{range}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{()))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-147-1.pdf}}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    ...
    \FloatTok{1.96} \SpecialCharTok{*}\NormalTok{ se, }\AttributeTok{ymax =}\NormalTok{ mean }\SpecialCharTok{+} \FloatTok{1.96} \SpecialCharTok{*}\NormalTok{ se)) }\SpecialCharTok{+} \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mean}\NormalTok{()) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"20 CI for N = 50"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylim}\NormalTok{(}\FunctionTok{range}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{()))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-148-1.pdf}}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    ...
    \FloatTok{1.96} \SpecialCharTok{*}\NormalTok{ se, }\AttributeTok{ymax =}\NormalTok{ mean }\SpecialCharTok{+} \FloatTok{1.96} \SpecialCharTok{*}\NormalTok{ se)) }\SpecialCharTok{+} \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mean}\NormalTok{()) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"20 CI for N = 100"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylim}\NormalTok{(}\FunctionTok{range}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{()))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-148-2.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{grid }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{5}\NormalTok{, }\DecValTok{5}\NormalTok{, }\FloatTok{0.1}\NormalTok{)}
\NormalTok{densDist }\OtherTok{\textless{}{-}} \FunctionTok{cbind}\NormalTok{(grid, }\FunctionTok{dnorm}\NormalTok{(grid), }\FunctionTok{sapply}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DecValTok{5}\NormalTok{,}
    \DecValTok{10}\NormalTok{), dt, }\AttributeTok{x =}\NormalTok{ grid))}
\FunctionTok{colnames}\NormalTok{(densDist) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"x"}\NormalTok{, }\StringTok{"normal"}\NormalTok{, }\FunctionTok{paste0}\NormalTok{(}\StringTok{"t"}\NormalTok{,}
    \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DecValTok{5}\NormalTok{, }\DecValTok{10}\NormalTok{)))}
...
\NormalTok{densDist }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ dens, }\AttributeTok{color =}\NormalTok{ dist)) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{xlab}\NormalTok{(}\StringTok{"x"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Densiteit"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/tdist-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{qt}\NormalTok{(}\FloatTok{0.975}\NormalTok{, }\AttributeTok{df =} \DecValTok{14}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{qt}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\FloatTok{0.95}\NormalTok{, }\FloatTok{0.975}\NormalTok{, }\FloatTok{0.995}\NormalTok{), }\AttributeTok{df =} \DecValTok{14}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(delta) }\SpecialCharTok{{-}} \FunctionTok{qt}\NormalTok{(}\FloatTok{0.975}\NormalTok{, }\AttributeTok{df =} \DecValTok{14}\NormalTok{) }\SpecialCharTok{*} \FunctionTok{sd}\NormalTok{(delta)}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(n)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(delta) }\SpecialCharTok{{-}} \FunctionTok{qt}\NormalTok{(}\FloatTok{0.995}\NormalTok{, }\AttributeTok{df =} \DecValTok{14}\NormalTok{) }\SpecialCharTok{*} \FunctionTok{sd}\NormalTok{(delta)}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(n)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{res}\SpecialCharTok{$}\NormalTok{ll }\OtherTok{\textless{}{-}}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{mean }\SpecialCharTok{{-}} \FunctionTok{qt}\NormalTok{(}\FloatTok{0.975}\NormalTok{, }\AttributeTok{df =}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{n }\SpecialCharTok{{-}} \DecValTok{1}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{se}
\NormalTok{res}\SpecialCharTok{$}\NormalTok{ul }\OtherTok{\textless{}{-}}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{mean }\SpecialCharTok{+} \FunctionTok{qt}\NormalTok{(}\FloatTok{0.975}\NormalTok{, }\AttributeTok{df =}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{n }\SpecialCharTok{{-}} \DecValTok{1}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{se}
...
\NormalTok{res }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ sample, }\AttributeTok{y =}\NormalTok{ mean, }\AttributeTok{color =}\NormalTok{ inside)) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_errorbar}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{ymin =}\NormalTok{ mean }\SpecialCharTok{{-}}
    \FunctionTok{qt}\NormalTok{(}\FloatTok{0.975}\NormalTok{, }\AttributeTok{df =} \DecValTok{9}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ se, }\AttributeTok{ymax =}\NormalTok{ mean }\SpecialCharTok{+} \FunctionTok{qt}\NormalTok{(}\FloatTok{0.975}\NormalTok{,}
    \AttributeTok{df =} \DecValTok{9}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ se)) }\SpecialCharTok{+} \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =}\NormalTok{ fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mean}\NormalTok{()) }\SpecialCharTok{+} \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"20 CI for N=10"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylim}\NormalTok{(}\FunctionTok{range}\NormalTok{(fem}\SpecialCharTok{$}\NormalTok{DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{()))}
\end{Highlighting}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:biInterpret}Interpretatie van 95\textbackslash\% betrouwbaarheidintervallen. Resultaten op basis van 20 gesimuleerde steekproeven. We zien in de figuur duidelijk dat het populatiegemiddelde vast is maar ongekend (volle zwarte lijn) en dat de bovengrens en ondergrens van betrouwbaarheidsintervallen voor het populatiegemiddelde varieert van steekproef tot steekproef.}]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/biInterpret-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-155-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{deltaSBP =} \FunctionTok{c}\NormalTok{(captopril}\SpecialCharTok{$}\NormalTok{deltaSBP, captoprilSamp}\SpecialCharTok{$}\NormalTok{deltaSBP),}
    \AttributeTok{shuffled =} \FunctionTok{rep}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\ConstantTok{FALSE}\NormalTok{, }\ConstantTok{TRUE}\NormalTok{), }\AttributeTok{each =} \DecValTok{15}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ shuffled, }\AttributeTok{y =}\NormalTok{ deltaSBP)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{stat\_summary}\NormalTok{(}\AttributeTok{fun.y =}\NormalTok{ mean,}
    \AttributeTok{geom =} \StringTok{"point"}\NormalTok{, }\AttributeTok{shape =} \DecValTok{19}\NormalTok{, }\AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{,}
    \AttributeTok{fill =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Difference (mm mercury)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-156-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-157-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-157-2.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{permH }\OtherTok{\textless{}{-}} \FunctionTok{expand.grid}\NormalTok{(}\FunctionTok{replicate}\NormalTok{(}\DecValTok{15}\NormalTok{, }\FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{), }\AttributeTok{simplify =} \ConstantTok{FALSE}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
...
\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{permMean =}\NormalTok{ muPerm) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ permMean)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =} \FunctionTok{mean}\NormalTok{(captopril}\SpecialCharTok{$}\NormalTok{deltaSBP),}
        \AttributeTok{col =} \StringTok{"blue"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-159-1.pdf}}
...
\NormalTok{tPerm }\OtherTok{\textless{}{-}} \FunctionTok{colMeans}\NormalTok{(deltaPerms)}\SpecialCharTok{/}\NormalTok{(}\FunctionTok{apply}\NormalTok{(deltaPerms, }\DecValTok{2}\NormalTok{,}
\NormalTok{    sd)}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(}\DecValTok{15}\NormalTok{))}
\NormalTok{tOrig }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(captopril}\SpecialCharTok{$}\NormalTok{deltaSBP)}\SpecialCharTok{/}\FunctionTok{sd}\NormalTok{(captopril}\SpecialCharTok{$}\NormalTok{deltaSBP) }\SpecialCharTok{*}
...
\NormalTok{tPermPlot }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{permT =}\NormalTok{ tPerm) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ permT)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..,}
    \AttributeTok{fill =}\NormalTok{ ..count..)) }\SpecialCharTok{+} \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =}\NormalTok{ tOrig,}
    \AttributeTok{col =} \StringTok{"blue"}\NormalTok{)}
\NormalTok{tPermPlot}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-160-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tPermPlot }\SpecialCharTok{+} \FunctionTok{stat\_function}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ dt, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{,}
    \AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{df =} \DecValTok{14}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-161-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{pt}\NormalTok{(stat, n }\SpecialCharTok{{-}} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
De drempelwaarde waarmee de \(p\)-waarde vergeleken wordt, wordt het \textbf{significantieniveau} genoemd en wordt voorgesteld door \(\alpha\).
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoTest-1} 
...
\NormalTok{deltaSim }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{(}\FunctionTok{rnorm}\NormalTok{(n }\SpecialCharTok{*}\NormalTok{ nsim, mu, sigma), }\AttributeTok{nrow =}\NormalTok{ n,}
    \AttributeTok{ncol =}\NormalTok{ nsim)}

\NormalTok{pSim }\OtherTok{\textless{}{-}} \FunctionTok{apply}\NormalTok{(deltaSim, }\DecValTok{2}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(x, mu, alternative) }\FunctionTok{t.test}\NormalTok{(x,}
    \AttributeTok{mu =}\NormalTok{ mu, }\AttributeTok{alternative =}\NormalTok{ alternative)}\SpecialCharTok{$}\NormalTok{p.value, }\AttributeTok{mu =}\NormalTok{ mu0,}
...
\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{pValue =}\NormalTok{ pSim) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ pValue)) }\SpecialCharTok{+} \FunctionTok{geom\_histogram}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{xlim}\NormalTok{(}\DecValTok{0}\NormalTok{,}
    \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-166-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-167-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{with}\NormalTok{(captopril, }\FunctionTok{t.test}\NormalTok{(SBPa, SBPb, }\AttributeTok{paired =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{alternative =} \StringTok{"less"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\item
  Door het pre-test/post-test design geen uitsluitsel kunnen geven of dit te wijten is aan de werking van het middel of aan een placebo effect. Er was geen goeie controle! Het gebrek aan een goeie controle is veelal een probleem bij pre-test/post-test designs.
\end{enumerate}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/captoTest2-1} 
...
\NormalTok{alpha }\OtherTok{\textless{}{-}} \FloatTok{0.05}
\NormalTok{n }\OtherTok{\textless{}{-}} \DecValTok{15}
\NormalTok{pvalsCor }\OtherTok{\textless{}{-}}\NormalTok{ pvalsInCor }\OtherTok{\textless{}{-}} \FunctionTok{array}\NormalTok{(}\DecValTok{0}\NormalTok{, nSim)}
...
\NormalTok{        pvalsInCor[i] }\OtherTok{\textless{}{-}} \FunctionTok{t.test}\NormalTok{(x, }\AttributeTok{alternative =} \StringTok{"less"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{p.value }\ControlFlowTok{else}\NormalTok{ pvalsInCor[i] }\OtherTok{\textless{}{-}} \FunctionTok{t.test}\NormalTok{(x, }\AttributeTok{alternative =} \StringTok{"greater"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{p.value}
\NormalTok{\}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{varXbarDelta }\OtherTok{\textless{}{-}}\NormalTok{ (vars[}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ vars[}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{] }\SpecialCharTok{{-}} \DecValTok{2} \SpecialCharTok{*}\NormalTok{ vars[}\DecValTok{1}\NormalTok{,}
    \DecValTok{2}\NormalTok{])}\SpecialCharTok{/}\DecValTok{15}
...
\begin{Highlighting}[]
\NormalTok{ap }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ trt, }\AttributeTok{y =}\NormalTok{ rel)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_jitter}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"Relatieve abundantie"}\NormalTok{) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/okselBox-1} 
...
\NormalTok{ap }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ rel)) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{facet\_grid}\NormalTok{(. }\SpecialCharTok{\textasciitilde{}}\NormalTok{ trt) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Relatieve abundantie"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/okselQQ-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{t.test}\NormalTok{(rel }\SpecialCharTok{\textasciitilde{}}\NormalTok{ trt, }\AttributeTok{data =}\NormalTok{ ap, }\AttributeTok{var.equal =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{t.test}\NormalTok{(rel }\SpecialCharTok{\textasciitilde{}}\NormalTok{ trt, }\AttributeTok{data =}\NormalTok{ ap, }\AttributeTok{var.equal =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \href{https://statomics.github.io/sbc/rmd/06-linearRegression.html}{script Hoofdstuk6}
\end{itemize}
...
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(}\FunctionTok{head}\NormalTok{(brca), }\AttributeTok{caption =} \StringTok{"Overzicht van de variabelen in de borstkanker dataset."}\NormalTok{,}
...
\NormalTok{brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \StringTok{""}\NormalTok{, }\AttributeTok{y =}\NormalTok{ S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{xlab}\NormalTok{(}\StringTok{""}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"S100A8 expressie"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/s100a8Boxplot-1} 
...
\NormalTok{brcaSubset[, }\SpecialCharTok{{-}}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{)] }\SpecialCharTok{|\textgreater{}}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaGenAl-1} 
...
\NormalTok{brcaSubset }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ ESR1, }\AttributeTok{y =}\NormalTok{ S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaSmooth-1} 
...
\NormalTok{brcaSubset }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ ESR1, }\AttributeTok{y =}\NormalTok{ S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{col =} \StringTok{"grey"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{,}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLin1-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLinRes-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLinDiag1-1} 
...
\NormalTok{brca }\OtherTok{\textless{}{-}}\NormalTok{ brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{log2S100A8 =} \FunctionTok{log2}\NormalTok{(S100A8), }\AttributeTok{log2ESR1 =} \FunctionTok{log2}\NormalTok{(ESR1))}

\NormalTok{lm2 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(log2S100A8 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ log2ESR1, brca)}

\NormalTok{brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{y =}\NormalTok{ log2S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{col =} \StringTok{"grey"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{,}
    \AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLogLin-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLogLin2-1} 
...
\NormalTok{g }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(lm2, }\AttributeTok{newdata =} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{log2ESR1 =}\NormalTok{ grid),}
...
\NormalTok{brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{y =}\NormalTok{ log2S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"ESR1 (log2)"}\NormalTok{) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLogLinPred1-1} 
...
\NormalTok{p1 }\OtherTok{\textless{}{-}}\NormalTok{ brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ ESR1, }\AttributeTok{y =}\NormalTok{ S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ grid, }\AttributeTok{y =}\NormalTok{ fit), newdata) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ grid,}
    \AttributeTok{y =}\NormalTok{ lwr), newdata, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ grid,}
    \AttributeTok{y =}\NormalTok{ upr), newdata, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"ESR1"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(}\StringTok{"S100A8"}\NormalTok{)}

\NormalTok{p2 }\OtherTok{\textless{}{-}}\NormalTok{ brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ ESR1, }\AttributeTok{y =}\NormalTok{ S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ grid, }\AttributeTok{y =}\NormalTok{ fit), newdata) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ grid,}
    \AttributeTok{y =}\NormalTok{ lwr), newdata, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ grid,}
    \AttributeTok{y =}\NormalTok{ upr), newdata, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{scale\_x\_continuous}\NormalTok{(}\AttributeTok{trans =} \StringTok{"log2"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{scale\_y\_continuous}\NormalTok{(}\AttributeTok{trans =} \StringTok{"log2"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"log2 ESR1"}\NormalTok{) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLogLinPred2-1} 
...
\begin{Highlighting}[]
\NormalTok{grid }\OtherTok{\textless{}{-}} \FunctionTok{log2}\NormalTok{(}\DecValTok{140}\SpecialCharTok{:}\DecValTok{4000}\NormalTok{)}
\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(lm2, }\AttributeTok{newdata =} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{log2ESR1 =}\NormalTok{ grid),}
...
\NormalTok{brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{y =}\NormalTok{ log2S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ grid,}
    \AttributeTok{y =}\NormalTok{ lwr), preddata, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ grid,}
    \AttributeTok{y =}\NormalTok{ upr), preddata, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLogLinPred3-1} 
...
\FunctionTok{library}\NormalTok{(NHANES)}
\NormalTok{fem }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Gender }\SpecialCharTok{==} \StringTok{"female"} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(DirectChol))}

\DecValTok{2}\SpecialCharTok{\^{}}\NormalTok{(fem }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{pull}\NormalTok{(DirectChol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mean}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{) }\SpecialCharTok{*} \FunctionTok{qnorm}\NormalTok{(}\FloatTok{0.975}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ (fem }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{pull}\NormalTok{(DirectChol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lmChol }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{\textasciitilde{}} \DecValTok{1}\NormalTok{, }\AttributeTok{data =}\NormalTok{ fem)}
\NormalTok{predInt }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(lmChol, }\AttributeTok{interval =} \StringTok{"prediction"}\NormalTok{,}
...
\DecValTok{2}\SpecialCharTok{\^{}}\NormalTok{(fem10 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{pull}\NormalTok{(DirectChol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mean}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{) }\SpecialCharTok{*} \FunctionTok{qnorm}\NormalTok{(}\FloatTok{0.975}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ (fem10 }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{pull}\NormalTok{(DirectChol) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{sd}\NormalTok{()))}
\end{Highlighting}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lmChol10 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(DirectChol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{\textasciitilde{}} \DecValTok{1}\NormalTok{, }\AttributeTok{data =}\NormalTok{ fem10)}
\NormalTok{predInt10 }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(lmChol10, }\AttributeTok{interval =} \StringTok{"prediction"}\NormalTok{,}
    \AttributeTok{newdata =} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{noPred =} \DecValTok{1}\NormalTok{))}
...
\begin{Highlighting}[]
\NormalTok{brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ log2S100A8, }\AttributeTok{x =}\NormalTok{ log2ESR1)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{color =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}
    ...
    \FunctionTok{geom\_segment}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{xend =}\NormalTok{ log2ESR1,}
        \AttributeTok{y =}\NormalTok{ log2S100A8, }\AttributeTok{yend =} \FunctionTok{mean}\NormalTok{(log2S100A8)), }\AttributeTok{lty =} \DecValTok{2}\NormalTok{,}
        \AttributeTok{color =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"ESR1 expressie (log2)"}\NormalTok{) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLogSSTot-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lm2\_df }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{log2S100A8 =}\NormalTok{ brca}\SpecialCharTok{$}\NormalTok{log2S100A8,}
    \AttributeTok{fitted =}\NormalTok{ lm2}\SpecialCharTok{$}\NormalTok{fitted.values, }\AttributeTok{log2ESR1 =}\NormalTok{ brca}\SpecialCharTok{$}\NormalTok{log2ESR1)}
\NormalTok{brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{y =}\NormalTok{ log2S100A8)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{y =}\NormalTok{ lm2}\SpecialCharTok{$}\NormalTok{fitted), }\AttributeTok{pch =} \DecValTok{2}\NormalTok{,}
        \AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{,}
    \AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{size =} \FloatTok{0.6}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_hline}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{yintercept =} \FunctionTok{mean}\NormalTok{(log2S100A8))) }\SpecialCharTok{+}
    \FunctionTok{geom\_segment}\NormalTok{(}\AttributeTok{data =}\NormalTok{ lm2\_df, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{xend =}\NormalTok{ log2ESR1,}
        \AttributeTok{y =}\NormalTok{ fitted, }\AttributeTok{yend =} \FunctionTok{mean}\NormalTok{(log2S100A8)), }\AttributeTok{lty =} \DecValTok{2}\NormalTok{,}
        \AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"S100A8 expressie (log2)"}\NormalTok{) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLogSSR-1} 
...
\NormalTok{brca }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{y =}\NormalTok{ lm2}\SpecialCharTok{$}\NormalTok{fitted), }\AttributeTok{pch =} \DecValTok{2}\NormalTok{,}
        \AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{,}
    \AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{size =} \FloatTok{0.6}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_segment}\NormalTok{(}\AttributeTok{data =}\NormalTok{ lm2\_df,}
    \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2ESR1, }\AttributeTok{xend =}\NormalTok{ log2ESR1, }\AttributeTok{y =}\NormalTok{ fitted,}
        \AttributeTok{yend =}\NormalTok{ log2S100A8), }\AttributeTok{lty =} \DecValTok{2}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(}\StringTok{"S100A8 expressie (log2)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"ESR1 expressie (log2)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLogSSE-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/brcaLymf-1} 
...
\begin{Highlighting}[]
\NormalTok{brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ node }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{as.factor}\NormalTok{(), }\AttributeTok{y =}\NormalTok{ age)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"node"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/boxNode-1} 
...
\NormalTok{sim\_df }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{node =} \FunctionTok{rep}\NormalTok{(brca}\SpecialCharTok{$}\NormalTok{node, }\DecValTok{9}\NormalTok{), }\AttributeTok{iid =} \FunctionTok{rnorm}\NormalTok{(}\DecValTok{9} \SpecialCharTok{*}
    \FunctionTok{nrow}\NormalTok{(brca), }\AttributeTok{sd =} \FunctionTok{sigma}\NormalTok{(lm3)), }\AttributeTok{sim =} \FunctionTok{rep}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{9}\NormalTok{, }\AttributeTok{each =} \DecValTok{32}\NormalTok{))}
\NormalTok{sim\_df }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ node, }\AttributeTok{y =}\NormalTok{ iid)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{facet\_wrap}\NormalTok{(. }\SpecialCharTok{\textasciitilde{}}\NormalTok{ sim) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(}\StringTok{"iid N(0,"}\NormalTok{, }\FunctionTok{round}\NormalTok{(}\FunctionTok{sigma}\NormalTok{(lm3)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{,}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/boxSim-1} 
...
\tightlist
\item
  \href{https://statomics.github.io/sbc/rmd/07-Anova.html}{script Hoofdstuk 7}
\end{itemize}
...
Het experiment is een \emph{volledige gerandomiseerd proefopzet}, \emph{``completely randomized design'' CRD}. In totaal worden 36 ratten (experimentele eenheden) at random toegekend aan de drie behandelingsgroepen (12 ratten per groep).
...
\NormalTok{prostacyclin }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prostBox-1} 
...
\begin{Highlighting}[]
\NormalTok{prostacyclin }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ prostac)) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{() }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prostBox2-1} 
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:prostacSST}Interpretatie van de kwadratensom van de behandeling (SST): de som van de kwadratische afwijkingen tussen de groepsgemiddelden (\textbackslash bar Y\_j) en het steekproefgemiddelde van de uitkomsten (\textbackslash bar Y)}]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/prostacSST-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:prostacSSTotSSE}Interpretatie van de totale kwadratensom (SSTot, som van de kwadratische afwijkingen tussen de uitkomsten Y\_\{i\} en het steekproefgemiddelde van de uitkomsten \textbackslash bar Y, links) en van residuele kwadratensom (SSE, som van de kwadratische afwijkingen tussen de uitkomsten Y\_\{i\} en de groepsgemiddelden \textbackslash bar Y\_j, rechts)}]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/prostacSSTotSSE-1.pdf}}
\caption{\label{fig:prostacSSTotSSE}Interpretatie van de totale kwadratensom (SSTot, som van de kwadratische afwijkingen tussen de uitkomsten \(Y_{i}\) en het steekproefgemiddelde van de uitkomsten \(\bar Y\), links) en van residuele kwadratensom (SSE, som van de kwadratische afwijkingen tussen de uitkomsten \(Y_{i}\) en de groepsgemiddelden \(\bar Y_j\), rechts)}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prostacF-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/ftheo-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{with}\NormalTok{(prostacyclin, }\FunctionTok{pairwise.t.test}\NormalTok{(prostac, dose, }\StringTok{"none"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\NormalTok{N) \{}
    \ControlFlowTok{if}\NormalTok{ (i}\SpecialCharTok{\%\%}\DecValTok{1000} \SpecialCharTok{==} \DecValTok{0}\NormalTok{)}
        \FunctionTok{cat}\NormalTok{(i, }\StringTok{"/"}\NormalTok{, N, }\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{with}\NormalTok{(prostacyclin, }\FunctionTok{pairwise.t.test}\NormalTok{(prostac, dose, }\AttributeTok{p.adjust.method =} \StringTok{"bonferroni"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{calpha\_bon\_t }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(object, level) \{}
    \FunctionTok{abs}\NormalTok{(}\FunctionTok{qt}\NormalTok{((}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ level)}\SpecialCharTok{/}\DecValTok{2}\SpecialCharTok{/}\FunctionTok{nrow}\NormalTok{(object}\SpecialCharTok{$}\NormalTok{linfct), object}\SpecialCharTok{$}\NormalTok{df))}
\NormalTok{\}}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prostacTukey-1} 
...
\NormalTok{    verwerp }\OtherTok{\textless{}{-}} \FunctionTok{min}\NormalTok{(}\FunctionTok{as.numeric}\NormalTok{(tests}\SpecialCharTok{$}\NormalTok{pvalues), }\AttributeTok{na.rm =}\NormalTok{ T) }\SpecialCharTok{\textless{}}
\NormalTok{        alpha}
    \ControlFlowTok{if}\NormalTok{ (verwerp)}
\NormalTok{        cnt }\OtherTok{\textless{}{-}}\NormalTok{ cnt }\SpecialCharTok{+} \DecValTok{1}
\NormalTok{\}}
...
\tightlist
\item
  \href{https://statomics.github.io/sbc/rmd/08-NonparametericStatistics.html}{script Hoofdstuk 8}
\end{itemize}
...
\NormalTok{chol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ group, }\AttributeTok{y =}\NormalTok{ cholest)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/cholBox-1} 
...
\begin{Highlighting}[]
\NormalTok{chol }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ cholest)) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{() }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/cholQQ-1} 
...
Als de nulhypothese waar is, met name dat de verdelingen van de cholesterolconcentraties gelijk zijn voor hartpatiënten en gezonde personen, dan zijn de groep-labels van de 10 personen niet informatief.
...
\subsubsection{\texorpdfstring{Verdeling van de statistiek onder \(H_0\)}{Verdeling van de statistiek onder H\_0}}\label{verdeling-van-de-statistiek-onder-h_0}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tStar }\OtherTok{\textless{}{-}} \FunctionTok{combn}\NormalTok{(n, nGroups[}\DecValTok{1}\NormalTok{], }\ControlFlowTok{function}\NormalTok{(g, y) \{}
    \FunctionTok{t.test}\NormalTok{(y[g], y[}\SpecialCharTok{{-}}\NormalTok{g])}\SpecialCharTok{$}\NormalTok{statistic}
\NormalTok{\}, }\AttributeTok{y =}\NormalTok{ chol }\SpecialCharTok{|\textgreater{}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{qplot}\NormalTok{(tStar, }\AttributeTok{bins =} \DecValTok{50}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\FunctionTok{expression}\NormalTok{(}\StringTok{"permutatiestatistiek t"}\SpecialCharTok{\^{}}\StringTok{"*"}\NormalTok{)) }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/tPermDist-1} 

}

\caption{Exacte permutatie nuldistributie van de Welch t-test voor het cholesterol voorbeeld. Bovenaan histogram van elke mogelijke waarde voor de test-statistiek. Onderaan een gewoon histogram met balkbreedtes gelijk aan 1. De geobserveerde waarde voor de test-statistiek is aangeduid met een rode verticale lijn.}\label{fig:tPermDist}
\end{figure}
...
We vinden dus een \(p\)-waarde van 0.0159. Aangezien \(p<5\%\), besluiten we op het \(5\%\) significantieniveau dat de distributies van de cholesterol concentraties niet gelijk zijn bij hartpatiënten en bij gezonde personen.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{metTies }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\DecValTok{403}\NormalTok{, }\DecValTok{507}\NormalTok{, }\DecValTok{507}\NormalTok{, }\DecValTok{610}\NormalTok{, }\DecValTok{651}\NormalTok{, }\DecValTok{651}\NormalTok{, }\DecValTok{651}\NormalTok{, }\DecValTok{830}\NormalTok{,}
    \DecValTok{900}\NormalTok{)}
\NormalTok{metTies }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{rank}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{u1Hlp }\OtherTok{\textless{}{-}} \FunctionTok{sapply}\NormalTok{(y1, }\ControlFlowTok{function}\NormalTok{(y1i, y2) \{}
\NormalTok{    y1i }\SpecialCharTok{\textgreater{}=}\NormalTok{ y2}
\NormalTok{\}, }\AttributeTok{y2 =}\NormalTok{ y2)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dna }\OtherTok{\textless{}{-}} \FunctionTok{read\_delim}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/statOmics/sbc/master/data/dna.txt"}\NormalTok{,}
...
\NormalTok{dna }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/dnaBox-1} 
...
\begin{Highlighting}[]
\NormalTok{dna }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ length)) }\SpecialCharTok{+} \FunctionTok{geom\_qq}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_qq\_line}\NormalTok{() }\SpecialCharTok{+}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/dnaQQ-1} 
...
\NormalTok{kwPerm }\OtherTok{\textless{}{-}} \FunctionTok{kruskal\_test}\NormalTok{(length }\SpecialCharTok{\textasciitilde{}}\NormalTok{ dose, }\AttributeTok{data =}\NormalTok{ dna, }\AttributeTok{distribution =} \FunctionTok{approximate}\NormalTok{(}\AttributeTok{nresample =} \FloatTok{1e+05}\NormalTok{))}
\NormalTok{kwPerm}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{probInd }\OtherTok{\textless{}{-}} \FunctionTok{combn}\NormalTok{(}\FunctionTok{levels}\NormalTok{(dna}\SpecialCharTok{$}\NormalTok{dose), }\DecValTok{2}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(x) \{}
\NormalTok{    test }\OtherTok{\textless{}{-}} \FunctionTok{wilcox.test}\NormalTok{(length }\SpecialCharTok{\textasciitilde{}}\NormalTok{ dose, dna }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{filter}\NormalTok{(dose }\SpecialCharTok{\%in\%}\NormalTok{ x))}
    \FunctionTok{return}\NormalTok{(test}\SpecialCharTok{$}\NormalTok{statistic}\SpecialCharTok{/}\FunctionTok{prod}\NormalTok{(nGroep[x]))}
\NormalTok{\})}

\FunctionTok{names}\NormalTok{(probInd) }\OtherTok{\textless{}{-}} \FunctionTok{combn}\NormalTok{(dna }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{pull}\NormalTok{(dose) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{levels}\NormalTok{(), }\DecValTok{2}\NormalTok{, paste, }\AttributeTok{collapse =} \StringTok{"vs"}\NormalTok{)}
...
Er zijn geen significante verschillen in de distributies van de comet-lengtes tussen de DMH behandelingen onderling (\(p=\) 0.72-0.82). DMH vertoont dus al bij de lage dosis genotoxische effecten. (Alle paarsgewijze testen werden gecorrigeerd voor multiple testing d.m.v. Holm's methode).
...
\tightlist
\item
  \href{https://statomics.github.io/sbc/rmd/09-categoricalDataAnalysis.html}{script Hoofdstuk 9}
\end{itemize}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/bernoulli-1} 
...
n \\
k \\
\end{array}\right)\) de binomiaalcoëfficient\footnote{Wees voorzichtig als je een binomiaalcoëfficient met een zakrekenmachine
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/binoms-1} 
...
\NormalTok{pUp }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}} \FunctionTok{pbinom}\NormalTok{(sUp }\SpecialCharTok{{-}} \DecValTok{1}\NormalTok{, n, pi0)}
\NormalTok{pUp}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/pitests-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/hamsterKooi-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{binom.test}\NormalTok{(}\AttributeTok{x =}\NormalTok{ f, }\AttributeTok{n =}\NormalTok{ f }\SpecialCharTok{+}\NormalTok{ g, }\AttributeTok{p =} \FloatTok{0.5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/chidist-1} 
...
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\DecValTok{2}\NormalTok{) }\ControlFlowTok{for}\NormalTok{ (j }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\DecValTok{2}\NormalTok{) expected[i, j] }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(brcaTab2[i,}
\NormalTok{    ]) }\SpecialCharTok{*} \FunctionTok{sum}\NormalTok{(brcaTab2[, j])}\SpecialCharTok{/}\FunctionTok{sum}\NormalTok{(brcaTab2)}
\NormalTok{expected}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x2 }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{((}\FunctionTok{abs}\NormalTok{(brcaTab2 }\SpecialCharTok{{-}}\NormalTok{ expected) }\SpecialCharTok{{-}} \FloatTok{0.5}\NormalTok{)}\SpecialCharTok{\^{}}\DecValTok{2}\SpecialCharTok{/}\NormalTok{expected)}
...
\begin{Highlighting}[]
\NormalTok{brca }\OtherTok{\textless{}{-}}\NormalTok{ brca }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{cancer =}\NormalTok{ cancer }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{as.factor}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{relevel}\NormalTok{(}\StringTok{"control"}\NormalTok{), }\AttributeTok{variant =}\NormalTok{ variant }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{as.factor}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
        \FunctionTok{relevel}\NormalTok{(}\StringTok{"pro/pro"}\NormalTok{))}

\NormalTok{brcaLogit }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(cancer }\SpecialCharTok{\textasciitilde{}}\NormalTok{ variant, }\AttributeTok{data =}\NormalTok{ brca, }\AttributeTok{family =}\NormalTok{ binomial)}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(multcomp)}
\NormalTok{posthoc }\OtherTok{\textless{}{-}} \FunctionTok{glht}\NormalTok{(brcaLogit, }\AttributeTok{linfct =} \FunctionTok{mcp}\NormalTok{(}\AttributeTok{variant =} \StringTok{"Tukey"}\NormalTok{))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{beetleModel }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(status }\SpecialCharTok{\textasciitilde{}}\NormalTok{ dose, }\AttributeTok{data =}\NormalTok{ beetles, }\AttributeTok{family =}\NormalTok{ binomial)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{beetlesTab }\OtherTok{\textless{}{-}} \FunctionTok{table}\NormalTok{(beetles) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{data.frame}\NormalTok{()}

\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{grid =} \FunctionTok{seq}\NormalTok{(}\FunctionTok{min}\NormalTok{(beetles}\SpecialCharTok{$}\NormalTok{dose), }\FunctionTok{max}\NormalTok{(beetles}\SpecialCharTok{$}\NormalTok{dose),}
    \FloatTok{0.1}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{piHat =} \FunctionTok{predict}\NormalTok{(beetleModel, }\AttributeTok{newdata =} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{dose =}\NormalTok{ grid),}
        \AttributeTok{type =} \StringTok{"response"}\NormalTok{)) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(grid, piHat)) }\SpecialCharTok{+} \FunctionTok{geom\_line}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{xlab}\NormalTok{(}\StringTok{"dose"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{ylab}\NormalTok{(}\StringTok{"probability (dead)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{geom\_text}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ dose }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.character}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.double}\NormalTok{(), }\AttributeTok{y =}\NormalTok{ status }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.character}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.double}\NormalTok{(), }\AttributeTok{label =}\NormalTok{ Freq), beetlesTab }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(status }\SpecialCharTok{==} \DecValTok{0}\NormalTok{)) }\SpecialCharTok{+} \FunctionTok{geom\_text}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ dose }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.character}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.double}\NormalTok{(), }\AttributeTok{y =}\NormalTok{ status }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.character}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.double}\NormalTok{(), }\AttributeTok{label =}\NormalTok{ Freq), beetlesTab }\SpecialCharTok{|\textgreater{}}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/keversFit-1} 
...
\tightlist
\item
  \href{https://statomics.github.io/sbc/rmd/10-MultipleRegression.html}{script Hoofdstuk 10}
\end{itemize}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/lpsaAll-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prosAdditiveFit-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prosLinDiag1-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lmVWS\_IntVS\_WS }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(lpsa }\SpecialCharTok{\textasciitilde{}}\NormalTok{ lcavol }\SpecialCharTok{+}\NormalTok{ lweight }\SpecialCharTok{+}\NormalTok{ svi }\SpecialCharTok{+}
\NormalTok{    svi}\SpecialCharTok{:}\NormalTok{lcavol }\SpecialCharTok{+}\NormalTok{ svi}\SpecialCharTok{:}\NormalTok{lweight, }\AttributeTok{data =}\NormalTok{ prostate)}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prosIntFit2-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lmVWS\_IntVW }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(lpsa }\SpecialCharTok{\textasciitilde{}}\NormalTok{ lcavol }\SpecialCharTok{+}\NormalTok{ lweight }\SpecialCharTok{+}\NormalTok{ svi }\SpecialCharTok{+}\NormalTok{ lcavol}\SpecialCharTok{:}\NormalTok{lweight,}
\NormalTok{    prostate)}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prosIntFit1-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cor}\NormalTok{(}\FunctionTok{cbind}\NormalTok{(prostate}\SpecialCharTok{$}\NormalTok{lcavol, prostate}\SpecialCharTok{$}\NormalTok{lweight, prostate}\SpecialCharTok{$}\NormalTok{lcavol }\SpecialCharTok{*}
\NormalTok{    prostate}\SpecialCharTok{$}\NormalTok{lweight))}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/vetScatter-1} 
...
\FunctionTok{plot}\NormalTok{(x, y, }\AttributeTok{xlim =} \FunctionTok{range}\NormalTok{(}\FunctionTok{c}\NormalTok{(x1, x2, x3)), }\AttributeTok{ylim =} \FunctionTok{range}\NormalTok{(}\FunctionTok{c}\NormalTok{(y1,}
\NormalTok{    y2, y3)))}
\FunctionTok{points}\NormalTok{(}\FunctionTok{c}\NormalTok{(x1[}\DecValTok{21}\NormalTok{], x2[}\DecValTok{21}\NormalTok{], x3[}\DecValTok{21}\NormalTok{]), }\FunctionTok{c}\NormalTok{(y1[}\DecValTok{21}\NormalTok{], y2[}\DecValTok{21}\NormalTok{],}
\NormalTok{    y3[}\DecValTok{21}\NormalTok{]), }\AttributeTok{pch =} \FunctionTok{as.character}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{3}\NormalTok{), }\AttributeTok{col =} \DecValTok{2}\SpecialCharTok{:}\DecValTok{4}\NormalTok{)}
...
\FunctionTok{legend}\NormalTok{(}\StringTok{"topleft"}\NormalTok{, }\AttributeTok{col =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{, }\AttributeTok{lty =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{, }\AttributeTok{legend =} \FunctionTok{paste}\NormalTok{(}\StringTok{"lm"}\NormalTok{,}
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/influential-1} 
...
\FunctionTok{plot}\NormalTok{(}\FunctionTok{cooks.distance}\NormalTok{(lmVWS), }\AttributeTok{type =} \StringTok{"h"}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}
    \DecValTok{1}\NormalTok{), }\AttributeTok{main =} \StringTok{"Additive model"}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \FunctionTok{qf}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\FunctionTok{length}\NormalTok{(lmVWS}\SpecialCharTok{$}\NormalTok{coef), }\FunctionTok{nrow}\NormalTok{(prostate) }\SpecialCharTok{{-}}
    \FunctionTok{length}\NormalTok{(lmVWS}\SpecialCharTok{$}\NormalTok{coef)), }\AttributeTok{lty =} \DecValTok{2}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(}\FunctionTok{cooks.distance}\NormalTok{(lmVWS\_IntVW), }\AttributeTok{type =} \StringTok{"h"}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}
    \DecValTok{1}\NormalTok{), }\AttributeTok{main =} \StringTok{"Model with lcavol:lweight interaction"}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \FunctionTok{qf}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\FunctionTok{length}\NormalTok{(lmVWS\_IntVW}\SpecialCharTok{$}\NormalTok{coef), }\FunctionTok{nrow}\NormalTok{(prostate) }\SpecialCharTok{{-}}
    \FunctionTok{length}\NormalTok{(lmVWS\_IntVW}\SpecialCharTok{$}\NormalTok{coef)), }\AttributeTok{lty =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prosCooksAdditiveInt-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prosDfBetasAdditive-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prosDfBetasInt-1} 
...
{\centering \includegraphics[width=1\linewidth]{Statistiek_2026_2027_files/figure-latex/prosInfl-1} 
...
Wanneer we het gewicht terugtransformeren naar de originele schaal zien we dat het prostaatgewicht voor deze patiënt 449.25 g bedraagt. De prostaat van een man heeft gewoonlijk een gewicht van 20-30 gram en het kan vergroten tot 50-100 gram.
...
\NormalTok{bpData }\OtherTok{\textless{}{-}}\NormalTok{ NHANES }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{filter}\NormalTok{(Age }\SpecialCharTok{\textgreater{}=} \DecValTok{6} \SpecialCharTok{\&}\NormalTok{ Age }\SpecialCharTok{\textless{}=} \DecValTok{18} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(BPSysAve) }\SpecialCharTok{\&}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-259-1.pdf}}
...
\tightlist
\item
  Is de Associatie tussen bloeddruk en leeftijd verschillend bij jongens en meisjes?
\end{enumerate}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(multcomp)}
\NormalTok{bpPosthoc }\OtherTok{\textless{}{-}} \FunctionTok{glht}\NormalTok{(mBp1, }\AttributeTok{linfct =} \FunctionTok{c}\NormalTok{(}\StringTok{"AgeMonths = 0"}\NormalTok{,}
    \StringTok{"AgeMonths + AgeMonths:Gendermale = 0"}\NormalTok{, }\StringTok{"AgeMonths:Gendermale = 0"}\NormalTok{))}
\NormalTok{bpPosthoc }\SpecialCharTok{|\textgreater{}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bpPosthocBI }\OtherTok{\textless{}{-}}\NormalTok{ bpPosthoc }\SpecialCharTok{|\textgreater{}}
...
De bloeddruk bij twee meisjes die in leeftijd verschillen is gemiddeld 0.06 mm Hg hoger per maand leeftijdsverschil bij het oudere meisje (p \textless\textless{} 0.001, 95\% BI {[}0.03, 0.09{]}.
De bloeddruk bij jongens die in leeftijd verschillen is gemiddeld 0.15 mm Hg hoger per maand leeftijdsverschil bij de oudere jongen. (p \textless\textless{} 0.001, 95\% BI {[}0.12, 0.18{]}.
Het gemiddelde bloeddrukverschil tussen personen in leeftijd verschillen is gemiddeld 0.09 mm Hg/maand hoger bij jongens dan meisjes (p \textless\textless{} 0.001, 95\% BI {[}0.05, 0.14{]}).
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-263-1.pdf}}
...
\begin{Highlighting}[]
\NormalTok{rats }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ treat, }\AttributeTok{y =}\NormalTok{ time)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_jitter}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{poison) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"time (h)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-264-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-265-1.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-265-2.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-265-3.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-265-4.pdf}}
...
\begin{Highlighting}[]
\NormalTok{rats }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ treat, }\AttributeTok{y =} \FunctionTok{log2}\NormalTok{(time))) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_jitter}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{poison)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-266-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rats2 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(time }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{log2}\NormalTok{() }\SpecialCharTok{\textasciitilde{}}\NormalTok{ poison }\SpecialCharTok{*}\NormalTok{ treat, rats)}
\FunctionTok{plot}\NormalTok{(rats2)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-266-2.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-266-3.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-266-4.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-266-5.pdf}}
...
\begin{Highlighting}[]
\NormalTok{rats }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ treat, }\AttributeTok{y =} \DecValTok{1}\SpecialCharTok{/}\NormalTok{time)) }\SpecialCharTok{+} \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{outlier.shape =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_jitter}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{poison) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"rate of dying (1/h)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-267-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-267-2.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-267-3.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-267-4.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-267-5.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-269-1.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-269-2.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-269-3.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-269-4.pdf}}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(multcomp)}
\NormalTok{comparisons }\OtherTok{\textless{}{-}} \FunctionTok{glht}\NormalTok{(rats4, }\AttributeTok{linfct =} \FunctionTok{mcp}\NormalTok{(}\AttributeTok{poison =} \StringTok{"Tukey"}\NormalTok{,}
    \AttributeTok{treat =} \StringTok{"Tukey"}\NormalTok{))}
...
\NormalTok{contrastNames }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"II{-}I"}\NormalTok{, }\StringTok{"III{-}I"}\NormalTok{, }\StringTok{"III{-}II"}\NormalTok{, }\StringTok{"B{-}A"}\NormalTok{,}
    \StringTok{"C{-}A"}\NormalTok{, }\StringTok{"D{-}A"}\NormalTok{, }\StringTok{"C{-}B"}\NormalTok{, }\StringTok{"D{-}B"}\NormalTok{, }\StringTok{"D{-}C"}\NormalTok{)}
\FunctionTok{axis}\NormalTok{(}\DecValTok{2}\NormalTok{, }\AttributeTok{at =} \FunctionTok{c}\NormalTok{(}\FunctionTok{length}\NormalTok{(contrastNames)}\SpecialCharTok{:}\DecValTok{1}\NormalTok{), }\AttributeTok{labels =}\NormalTok{ contrastNames,}
    \AttributeTok{las =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-271-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ExploreModelMatrix}\SpecialCharTok{::}\FunctionTok{VisualizeDesign}\NormalTok{(rats, }\SpecialCharTok{\textasciitilde{}}\NormalTok{poison }\SpecialCharTok{*}
\NormalTok{    treat)}\SpecialCharTok{$}\NormalTok{plot}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-272-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{comparisonsInt }\OtherTok{\textless{}{-}} \FunctionTok{glht}\NormalTok{(rats3, }\AttributeTok{linfct =} \FunctionTok{c}\NormalTok{(}\StringTok{"poisonII = 0"}\NormalTok{,}
    \StringTok{"poisonIII = 0"}\NormalTok{, }\StringTok{"poisonIII {-} poisonII = 0"}\NormalTok{, }\StringTok{"poisonII + poisonII:treatB = 0"}\NormalTok{,}
    \StringTok{"poisonIII + poisonIII:treatB = 0"}\NormalTok{, }\StringTok{"poisonIII + poisonIII:treatB {-} poisonII{-} poisonII:treatB = 0"}\NormalTok{,}
    \StringTok{"poisonII + poisonII:treatC = 0"}\NormalTok{, }\StringTok{"poisonIII + poisonIII:treatC = 0"}\NormalTok{,}
    \StringTok{"poisonIII + poisonIII:treatC {-} poisonII{-} poisonII:treatC = 0"}\NormalTok{,}
    \StringTok{"poisonII + poisonII:treatD = 0"}\NormalTok{, }\StringTok{"poisonIII + poisonIII:treatD = 0"}\NormalTok{,}
    \StringTok{"poisonIII + poisonIII:treatD {-} poisonII{-} poisonII:treatD = 0"}\NormalTok{,}
    \StringTok{"treatB = 0"}\NormalTok{, }\StringTok{"treatC = 0"}\NormalTok{, }\StringTok{"treatD = 0"}\NormalTok{, }\StringTok{"treatC {-} treatB = 0"}\NormalTok{,}
    \StringTok{"treatD {-} treatB = 0"}\NormalTok{, }\StringTok{"treatD {-} treatC = 0"}\NormalTok{, }\StringTok{"treatB + poisonII:treatB = 0"}\NormalTok{,}
    \StringTok{"treatC + poisonII:treatC = 0"}\NormalTok{, }\StringTok{"treatD + poisonII:treatD = 0"}\NormalTok{,}
    ...
    \StringTok{"treatB + poisonIII:treatB = 0"}\NormalTok{, }\StringTok{"treatC + poisonIII:treatC = 0"}\NormalTok{,}
    \StringTok{"treatD + poisonIII:treatD = 0"}\NormalTok{, }\StringTok{"treatC + poisonIII:treatC {-} treatB {-} poisonIII:treatB = 0"}\NormalTok{,}
...
\NormalTok{contrastNames }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\FunctionTok{rep}\NormalTok{(LETTERS[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{], }\AttributeTok{each =} \DecValTok{3}\NormalTok{),}
    \FunctionTok{rep}\NormalTok{(}\FunctionTok{apply}\NormalTok{(}\FunctionTok{combn}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"I"}\NormalTok{, }\StringTok{"II"}\NormalTok{, }\StringTok{"III"}\NormalTok{), }\DecValTok{2}\NormalTok{)[}\DecValTok{2}\SpecialCharTok{:}\DecValTok{1}\NormalTok{, ],}
        \DecValTok{2}\NormalTok{, paste, }\AttributeTok{collapse =} \StringTok{"{-}"}\NormalTok{), }\DecValTok{4}\NormalTok{), }\AttributeTok{sep =} \StringTok{": "}\NormalTok{),}
    \FunctionTok{paste}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"I"}\NormalTok{, }\StringTok{"II"}\NormalTok{, }\StringTok{"III"}\NormalTok{), }\AttributeTok{each =} \DecValTok{6}\NormalTok{), }\FunctionTok{rep}\NormalTok{(}\FunctionTok{apply}\NormalTok{(}\FunctionTok{combn}\NormalTok{(}\FunctionTok{c}\NormalTok{(LETTERS[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{]),}
        \DecValTok{2}\NormalTok{)[}\DecValTok{2}\SpecialCharTok{:}\DecValTok{1}\NormalTok{, ], }\DecValTok{2}\NormalTok{, paste, }\AttributeTok{collapse =} \StringTok{"{-}"}\NormalTok{), }\DecValTok{3}\NormalTok{), }\AttributeTok{sep =} \StringTok{": "}\NormalTok{))}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-273-1.pdf}}
...
\tightlist
\item
  de effectgrootte voor de paarsgewijze vergelijkingen tussen de verschillende giffen (II-I, III-I en III-II) te schatten door ze uit te middelen over alle behandelingen (A, B, C, en D), en,
\item
  de effectgrootte voor de paarsgewijze vergelijkingen tussen de verschillende behandelingen (B-A, C-A, D-A, C-B, D-B en D-C) te schatten door ze uit te middelen over alle giffen (I, II, III).
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{contrasts }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"poisonII + 1/4*poisonII:treatB + 1/4*poisonII:treatC + 1/4*poisonII:treatD = 0"}\NormalTok{,}
...
\pandocbounded{\includegraphics[keepaspectratio]{Statistiek_2026_2027_files/Statistiek_2026_2027_files/figure-latex/unnamed-chunk-274-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{Additive\_coef =} \FunctionTok{summary}\NormalTok{(comparisons)}\SpecialCharTok{$}\NormalTok{test}\SpecialCharTok{$}\NormalTok{coef,}
    \AttributeTok{Additive\_se =} \FunctionTok{summary}\NormalTok{(comparisons)}\SpecialCharTok{$}\NormalTok{test}\SpecialCharTok{$}\NormalTok{sigma,}
    \AttributeTok{Int\_coef =} \FunctionTok{summary}\NormalTok{(comparisonsInt2)}\SpecialCharTok{$}\NormalTok{test}\SpecialCharTok{$}\NormalTok{coef,}
    \AttributeTok{int\_se =} \FunctionTok{summary}\NormalTok{(comparisonsInt2)}\SpecialCharTok{$}\NormalTok{test}\SpecialCharTok{$}\NormalTok{sigma) }\SpecialCharTok{|\textgreater{}}
