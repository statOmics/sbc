...
```{r pop2Samp2PopH4, out.width='80%',fig.asp=.8, fig.align='center',echo=FALSE}
if ("pi"%in%ls()) rm("pi")
...
Schatting van verwachte waarde van X of gemiddelde

$$
E[X] = \int\limits_{x \in \Omega} x f(x) dx
$$

in de steekproef:

$$
 \bar x = \sum\limits_{i=1}^n x_i \frac{1}{n}
$$
...
1. Interesse in variabiliteit:
    ...
    - Chemici beogen vaak een reactieproces te bekomen voor de productie van stoffen waar de variabiliteit (constante kwaliteit).
...
$$
E[(X-E[X])^2] = \int\limits_{ x \in \Omega} (x-E[X])^2 f(x) dx
$$
 in de steekproef:

$$
s_X^2= \sum\limits_{i=1}^n (X-\bar X)^2\frac{1}{n-1}
$$
...
2. Maake een object p2 met de QQ-plot
...
NHANES%>% filter(Age >= 18 & Gender=="female") %>%
  ...
  annotate(geom="text",
           ...
           label=c("III: +","II: -","IV: -","I: +"),color="red",size=7)
```
