---
title: "9. Categorische Data Analyse"
...
- Op `r n` ongeboren kinderen werden `r boys` jongens geobserveerd.
...
- Formeel: populatie van ongeboren kinderen
...
- Binaire data $\longrightarrow$ Bernoulli verdeling:
  \begin{eqnarray*}
...
- Variantie van Bernoulli data bepaald door kans $\pi$.
$$\text{Var}[X_i]=\pi (1-\pi).$$
...
- Saksenstudie heel grote steekproef: `r n` onafhankelijke observaties.
...
- BI: [`r  paste(format(pi + c(-1,1)*qnorm(.975)*se, digits=3), collapse=", ")`]
...
Er is significant meer kans dat een ongeboren kind mannelijk dan vrouwelijk is (p=`r round(pval,3)`). De kans dat een ongeboren kind mannelijk is bedraagt `r round(pi, 3)` (95% BI [`r  paste(format(pi + c(-1,1)*qnorm(.975)*se, digits=3), collapse=", ")`]).

## Exacte test: Binomiale test
...
knitr::kable(hamsterTot,caption="Kruistabel van partnerkeuze bij dwerghamster.",booktabs = TRUE, format="markdown")
```
...
\begin{equation*}
\widehat{\text{ARV}}=\hat\pi_2-\hat\pi_1=\frac{e+f}{n}-\frac{e+g}{n}=\frac{f-g}{n}
\end{equation*}
...
controles is bijgevolg $(89/800)/(56/572)=1.14$.
...
- Toetsingsgrootheid  groot $\longrightarrow$ indicatie voor
afwijking van $H_0$.
- De p-waarde voor een 2-zijdige toets is
kans om grotere waarde voor toetsingsgrootheid te observeren dan
geobserveerde waarde $x^2$ als nulhypothese waar is $\longrightarrow$ kans dat $\chi^2_1$-verdeelde toevalsveranderlijke waarden groter dan $x^2$
aanneemt.
...
- Omdat observaties $O_{ij}$ discrete getallen zijn, kan
toetsingsgrootheid $X^2$ slechts discrete waarden aannemen en kan
continue verdeling zoals $\chi^2_1$-verdeling slechts een benadering zijn.
...
- Zelfs met continuïteitscorrectie is $\chi^2_1$ benadering slechts
...
- De continuïteitscorrectie wordt meestal niet gebruikt bij meer dan 2 rijen of
kolommen.
...
Logistisch regressiemodel dat log odds modelleert in functie van dosis $x_i$:
...
- Merk op: heel sterke extrapolatie: minimum dosis in de dataset `r min(beetles$dose)` mg/l.
...
- We besluiten dat dit effect heel significant is ($p=$ `r round(summary(beetleModel)$coef[2,4],3)`).
