...
```{example, name="NHANES studie",label="nhanesExConcepten"}
&nbsp;
```
```{r,echo=FALSE}
library(NHANES)
...
```{r nhanesConcepten, tidy=FALSE,echo=FALSE}
library(NHANES)
knitr::kable(
...
### Achtergrond Normale verdeling {#subsec:normalcalc}
...
```{r continu, fig.cap='Bovenste rij: dichtheidsfunctie van een Normale verdeling met gemiddelde $\\mu=80$ en standaardafwijking $\\sigma=12$, met de rode oppervlakte onder de curve die links de kans $P(X<64)$, in het midden de kans $P(X<80)$ en rechts de kans $P(64<X<80)$ voorstelt. Onderste rij: overeenkomstige distributiefunctie $F(x)=P(X\\leq x)$, met telkens de waarde van $x$ en de bijhorende kans aangeduid.', out.width='100%', fig.asp=.7, fig.align='center', echo=FALSE}
mu <- 80
sigma <- 12
A <- 64
B <- 80
...
```{definition, name="distributiefunctie"}
&nbsp;
```
...
```{definition, name="referentie-interval"}
&nbsp;
```
